...
import type { SkeletonFrame } from "@/lib/swing/types";
...
export function Swing3D({ frames, playing = true, label }: Props) {
  ...
  return (
    <div className="swing-3d">
      <canvas ref={canvasRef} className="swing-3d__canvas" aria-label="3D 스윙" />
      <p className="swing-3d__hint">
        {label ?? "드래그하면 돌아갑니다"} · 앞면+뒷면을 올리면 깊이가 생깁니다
      </p>
    </div>
  );
}
