#!/usr/bin/env python3
"""Re-download Galvin Green pale colourway PDP images from Shopify CDN (no greymat).

Prior rembg/soft greymat washed white pants (Noah) and sand apparel onto
#e7e7e7. Restore pristine CDN bytes — same approach as redownload-bs-pale.py.
"""
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": UA,
                "Accept": "image/*",
                "Referer": "https://www.galvingreen.com/",
            },
        )
...
def download_handle(handle: str, urls: list[str]) -> tuple[str, int, int]:
    ...
    for i, url in enumerate(urls[:6], start=1):
        ...
        rembg_only_greymat(dest_dir / f"{i}.jpg")
        ok += 1
...
def main() -> None:
    ...
    if args.only:
        ...
        handles = sorted(pale_gg_handles(raw))

    jobs: list[tuple[str, list[str]]] = []
    for h in handles:
        row = by_handle.get(h) or {}
        if not is_pale_gg_row(row) and not args.only:
            continue
        urls = list(row.get("images") or [])
        jobs.append((h, urls))

    print(
        f"GG pale redownload handles={len(jobs)} workers={args.workers}",
        flush=True,
    )
