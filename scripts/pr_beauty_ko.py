#!/usr/bin/env python3
"""Curated Korean copy for Prada Beauty."""
from __future__ import annotations

import json
from pathlib import Path

_DATA = Path(__file__).resolve().parents[1] / "src/data/pr/pr-beauty-ko-copy.json"
...
def seed_beauty_cache(cache: dict[str, str]) -> int:
    n = 0
...
def beauty_text_ko(text: str | None) -> str | None:
    s = (text or "").strip()
    if not s:
        return ""
    for mapping in (TITLE_KO, COLOR_KO, DESCRIPTION_KO, DETAIL_KO, MATERIAL_KO):
