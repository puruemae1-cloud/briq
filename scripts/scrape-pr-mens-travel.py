#!/usr/bin/env python3
"""Scrape Prada GB men's travel (~32) + PDP images.

Source hub: https://www.prada.com/gb/en/mens/travel/c/10175EU
PLP membership uses Algolia CategoriesEnriched ``10175EU|false|false``.
...
OUT_RAW = ROOT / "src/data/pr/pr-mens-travel-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-mens-travel-pdp-cache.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"

BASE = "https://www.prada.com"
HUB_URL = f"{BASE}/gb/en/mens/travel/c/10175EU"
HUB_CID = "10175EU"
...
LEAVES: list[tuple[str, str, str]] = [
    ("pr-men-travel-bags", "Travel bags", "10178EU"),
    ("pr-men-luggage-carry-on", "Luggage and carry-on", "10176EU"),
    ("pr-men-travel-accessories", "Travel accessories", "10177EU"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid in LEAVES}
BC_TO_LEAF = {
    "Travel bags": "pr-men-travel-bags",
    "Luggage and carry-on": "pr-men-luggage-carry-on",
    "Luggage and carry on": "pr-men-luggage-carry-on",
    "Travel accessories": "pr-men-travel-accessories",
    "Pouches": "pr-men-travel-accessories",
    "Trolleys": "pr-men-luggage-carry-on",
    "Duffle bags": "pr-men-travel-bags",
    "Duffel bags": "pr-men-travel-bags",
}
LEAF_SLUG = {
    "pr-men-travel-bags": "travel-bags",
    "pr-men-luggage-carry-on": "luggage-and-carry-on",
    "pr-men-travel-accessories": "travel-accessories",
}

PARENT_COLS = ["prada", "prada-bags", "pr-mens-handbags", "pr-men-travel"]
MAX_WORKERS = 6
IMG_WORKERS = 8
MAX_IMAGES = 12
...
def infer_leaf_from_name(name: str) -> str | None:
    ...
    if any(x in n for x in ("trolley", "suitcase", "cabin", "carry-on", "carry on")):
        return "pr-men-luggage-carry-on"
    if any(x in n for x in ("pouch", "cosmetic", "toiletry", "organizer", "necessaire")):
        return "pr-men-travel-accessories"
    if any(x in n for x in ("duffel", "duffle", "travel bag", "weekender")):
        return "pr-men-travel-bags"
    return None
...
def primary_leaf(cols: list[str], hit: dict) -> str:
    ...
    for lid, _label, _cid in LEAVES:
        if lid in cols:
            return lid
    name = ((hit.get("ProductName") or {}).get("en_GB") or "")
    return infer_leaf_from_name(name) or "pr-men-travel-bags"
...
def main() -> None:
    ...
    print("=== Prada men's travel hub (Algolia)", flush=True)
    ...
    for lid, label, cid in LEAVES:
        ...
        plp_meta[lid] = {
            ...
            "categoryCode": cid,
            "count": n,
            "url": f"{BASE}/gb/en/mens/travel/{LEAF_SLUG[lid]}/c/{cid}",
        }
