#!/usr/bin/env python3
"""Scrape Dior GB Women's Fashion Jewellery via Playwright WebKit."""
...
OUT_RAW = ROOT / "src/data/di/di-women-jewelry-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-women-jewelry-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-women-jewelry-pdp-cache.json"
HUB = f"{BASE}/{LANG}/fashion/womens-fashion/fashion-jewellery/jewellery"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_WOMEN_JEWELRY, leaf["id"]],
...
def main() -> None:
    ...
    if args.stage == "all":
        leaves = list(WOMEN_JEWELRY_LEAVES)
    else:
        leaves = [
            L
            for L in WOMEN_JEWELRY_LEAVES
            if L["id"] == args.stage or L.get("stage") == str(args.stage)
        ]
        if not leaves:
            raise SystemExit(f"unknown stage/leaf: {args.stage}")

    save_json(OUT_LEAVES, WOMEN_JEWELRY_LEAVES)
        ...
        for leaf in leaves:
            ...
            for i, hit in enumerate(hits, start=1):
                ...
                if prev:
                    ...
                    if leaf["id"] in ("di-women-jewelry-all",) and prev.get("leafId"):
                        pl = prev["leafId"]
                        if pl not in ("di-women-jewelry-all",):
                            row["leafId"] = pl
