#!/usr/bin/env python3
"""Scrape Prada GB women's shoes (~193) in stages.

Hub: https://www.prada.com/gb/en/womens/shoes/c/10070EU
PLP set = Algolia CategoriesEnriched ``10070EU|false|false``.

  python3 scripts/scrape-pr-womens-shoes.py --stage 1 --stages 2
  python3 scripts/scrape-pr-womens-shoes.py --refresh-sizes
"""
...
OUT_RAW = ROOT / "src/data/pr/pr-womens-shoes-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-womens-shoes-pdp-cache.json"
SEED = ROOT / "src/data/pr/pr-womens-shoes-hub-seed.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"
PROGRESS = ROOT / "src/data/pr/pr-womens-shoes-stage-progress.json"

BASE = "https://www.prada.com"
HUB_URL = f"{BASE}/gb/en/womens/shoes/c/10070EU"
HUB_CID = "10070EU"
...
# Official leaf PLPs under women's shoes (prada.com GB hub chips)
LEAVES: list[tuple[str, str, str]] = [
    ("pr-women-ankle-boots-boots", "Ankle boots and boots", "10071EU"),
    ("pr-women-loafers-lace-ups", "Loafers and lace-ups", "10074EU"),
    ("pr-women-pumps-ballerinas", "Pumps and ballerinas", "10075EU"),
    ("pr-women-sneakers", "Sneakers", "10078EU"),
    ("pr-women-sandals-mules", "Sandals and mules", "10720EU"),
    ("pr-women-new-formal", "New formal", "10686EU"),
    ("pr-women-chocolate", "Chocolate", "10684EU"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid in LEAVES}
BC_TO_LEAF = {
    "Ankle boots and boots": "pr-women-ankle-boots-boots",
    "Loafers and lace-ups": "pr-women-loafers-lace-ups",
    "Pumps and ballerinas": "pr-women-pumps-ballerinas",
    "Sneakers": "pr-women-sneakers",
    "Sandals and mules": "pr-women-sandals-mules",
    "New formal": "pr-women-new-formal",
    "New Formal": "pr-women-new-formal",
    "Chocolate": "pr-women-chocolate",
}
LEAF_SLUG = {
    "pr-women-ankle-boots-boots": "ankle-boots-and-boots",
    "pr-women-loafers-lace-ups": "loafers-and-lace-ups",
    "pr-women-pumps-ballerinas": "pumps-and-ballerinas",
    "pr-women-sneakers": "sneakers",
    "pr-women-sandals-mules": "sandals-and-mules",
    "pr-women-new-formal": "new-formal",
    "pr-women-chocolate": "chocolate",
}

PARENT_COLS = ["prada", "prada-shoes", "pr-women-shoes"]
MAX_WORKERS = 4
IMG_WORKERS = 4
MAX_IMAGES = 10
...
def collections_for(hit: dict) -> list[str]:
    ...
    for cid in hit.get("Categories") or []:
        ...
        if lid:
            cols.add(lid)
    if not any(c.startswith("pr-women-") and c != "pr-women-shoes" for c in cols):
...
def main() -> None:
    ...
    for lid, label, cid in LEAVES:
        ...
        plp_meta[lid] = {
            ...
            "categoryCode": cid,
            "count": n,
            "url": f"{BASE}/gb/en/womens/shoes/{slug}/c/{cid}",
        }
