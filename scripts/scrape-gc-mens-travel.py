#!/usr/bin/env python3
"""Scrape Gucci UK men's travel bags + PDP images.

Mirrors scrape-gc-womens-travel.py + modern PLP supplement from
scrape-gc-mens-wallets.py. Writes gc-mens-travel-catalog-raw.json;
images → public/products/gc-pdp/.

Official PLP (~69):
https://www.gucci.com/uk/en_gb/ca/men/travel-bags-for-men-c-men-bags-trolleys
"""
...
OUT_RAW = ROOT / "src/data/gc/gc-mens-travel-catalog-raw.json"
...
WARM_URL = (
    ...
    "travel-bags-for-men-c-men-bags-trolleys"
)

# Official travel leaves → Briq collection ids
COLLECTIONS: list[tuple[str, str, str]] = [
    ("gc-men-trolley", "Trolley", "men-travel-trolley"),
    (
        "gc-men-weekend-duffle",
        "Weekend bags & Duffle bags",
        "men-carry-ons",
    ),
    (
        "gc-men-travel-accessories",
        "Accessories",
        "men-portfolios-pouches",
    ),
    (
...
ALL_TRAVEL_CODE = "men-bags-trolleys"
PARENT_COLL = "gc-men-travel"
PARENT_COLLS = (
    "gc-men-travel",
    "gc-accessories-mens",
    "gucci-accessories",
...
MODERN_PLP_PATHS: list[tuple[str, str]] = [
    ...
    (
        "gc-men-trolley",
        "/uk/en_gb/ca/men/travel-bags-for-men/"
        "trolley-c-men-travel-trolley",
    ),
    (
        "gc-men-weekend-duffle",
        "/uk/en_gb/ca/men/travel-bags-for-men/"
        "weekend-bags-and-duffle-bags-c-men-carry-ons",
    ),
    (
        "gc-men-travel-accessories",
        "/uk/en_gb/ca/men/travel-bags-for-men/"
        "accessories-for-men-c-men-portfolios-pouches",
    ),
    (
        "gc-men-hard-shell-luggage",
        "/uk/en_gb/ca/men/travel-bags-for-men/"
        "harded-shell-luggage-for-men-c-men-hard-sided",
    ),
]
...
def fetch_modern_plp_style_codes(
    ...
    """Collect styleCodes from the live PLP (HTML + RSC).

    Legacy /c/productgrid under-counts vs live trolley/accessories leaves.
    """
    seen: dict[str, None] = {}
    stagnant = 0
    for page in range(0, max_page + 1):
        ...
        for accept, extra in (
            ...
            ("text/x-component", {"RSC": "1"}),
        ):
            try:
                r = s.get(
                    url,
                    headers={**headers_html(), "Accept": accept, **extra},
                    impersonate="chrome124",
                    timeout=90,
                )
                if r.status_code == 200:
...
def leaf_from_pdp_url(url: str) -> str | None:
    u = (url or "").lower()
    if "trolley" in u or "cabin" in u or "check-in" in u or "checkin" in u:
        return "gc-men-trolley"
    if "hard" in u or "shell" in u or "suitcase" in u:
        return "gc-men-hard-shell-luggage"
    if "duffle" in u or "duffel" in u or "weekend" in u or "carry-on" in u:
        return "gc-men-weekend-duffle"
    if "pouch" in u or "portfolio" in u or "accessor" in u or "toiletry" in u:
        return "gc-men-travel-accessories"
    return None
...
def resolve_pdp_link(
    ...
    for folder in (
        "travel-bags-for-men/trolley",
        "travel-bags-for-men/weekend-bags-and-duffle-bags",
        "travel-bags-for-men/accessories-for-men",
        "travel-bags-for-men/harded-shell-luggage-for-men",
        "bags-for-men/duffle-bags-for-men",
        "travel-bags-for-men",
    ):
...
def main() -> None:
    ...
    print(f"=== {PARENT_COLL} ({ALL_TRAVEL_CODE})", flush=True)
    all_items = fetch_grid_category(s, ALL_TRAVEL_CODE)
    plp_meta[PARENT_COLL] = {
        "label": "Travel Bags / View All",
        "categoryCode": ALL_TRAVEL_CODE,
    ...
    for coll_id, plp_path in MODERN_PLP_PATHS:
        ...
        if coll_id == PARENT_COLL:
            plp_meta[PARENT_COLL]["count"] = max(
    ...
    parent_n = sum(1 for p in products if PARENT_COLL in p["collections"])
    print(f"  {PARENT_COLL}: {parent_n}", flush=True)
