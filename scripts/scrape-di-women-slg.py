#!/usr/bin/env python3
"""Scrape Dior GB Women's Small Leather Goods via Playwright WebKit.

Official leaves under Accessories → Dior → 여성용 (~296 SKUs). Run in 2
stages with a pause between for machine stability:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \\
    python3 scripts/scrape-di-women-slg.py --stage 1
  # pause
  python3 scripts/scrape-di-women-slg.py --stage 2
...
OUT_RAW = ROOT / "src/data/di/di-women-slg-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-women-slg-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-women-slg-pdp-cache.json"

HUB = f"{BASE}/{LANG}/fashion/womens-fashion/small-leather-goods/all-small-leather-goods"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_WOMEN_SLG, leaf["id"]],
