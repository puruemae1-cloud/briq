#!/usr/bin/env python3
"""Scrape Gucci UK women's shoes + PDP images.

Mirrors scrape-gc-womens-rtw.py: productgrid leaves + catalog API + DarkGray
images into public/products/gc-pdp/. Writes gc-shoes-catalog-raw.json.
"""
...
OUT_RAW = ROOT / "src/data/gc/gc-shoes-catalog-raw.json"
...
WARM_URL = f"{BASE}/uk/en_gb/ca/women/shoes-for-women-c-women-shoes"

# Official Shoes leaves (Gucci UK women shoes nav) → Briq collection ids
# categoryCode is the URL segment after -c-
COLLECTIONS: list[tuple[str, str, str]] = [
    ("gc-women-sneakers", "Trainers", "women-shoes-sneakers"),
    (
        "gc-women-moccasins",
        "Moccasins & Lace-Ups",
        "women-shoes-moccasins-and-loafers",
    ),
    ("gc-women-slippers-mules", "Slippers & Mules", "women-shoes-slides-mules"),
    ("gc-women-sandals", "Sandals", "women-shoes-sandals"),
    ("gc-women-slides", "Slides", "women-shoes-slides-thongs"),
    ("gc-women-pumps", "Pumps", "women-shoes-pumps"),
    ("gc-women-ballet-flats", "Ballet Flats", "women-shoes-ballerinas"),
    (
        "gc-women-boots",
        "Boots & Ankle Boots",
        "women-shoes-boots-and-booties",
    ),
]

ALL_SHOES_CODE = "women-shoes"
PARENT_COLL = "gc-women-shoes"
PARENT_COLLS = ("gc-women-shoes", "gc-shoes-womens", "gucci-shoes")
MAX_WORKERS = 6
IMG_WORKERS = 8
...
def normalize_shoe_size(size: str) -> str:
    """Catalog uses 34+ for half sizes; official chart / PDP use 34.5."""
    s = (size or "").strip()
...
def extract_sizes(catalog: dict | None) -> list[dict]:
    """Return unique shoe size rows (IT numeric / half sizes)."""
    out: list[dict] = []
    seen: set[str] = set()
    for v in (catalog or {}).get("variants") or []:
        size = normalize_shoe_size(str(v.get("sizeDescription") or "").strip())
