#!/usr/bin/env python3
"""Scrape Prada GB Fine Jewelry + PDP images.

Hub: https://www.prada.com/gb/en/fine-jewelry/categories/c/10628EU
Official leaf PLPs under Categories:
  - Bracelets              10629EU
  - Necklaces              10630EU
  - Rings                  10631EU
  - Earrings and brooches  10632EU

Product set = union of hub + all leaf CIDs.
SKUs without GBP price are kept in raw JSON but omitted at catalogue build time.
"""
...
OUT_RAW = ROOT / "src/data/pr/pr-fine-jewelry-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-fine-jewelry-pdp-cache.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"

BASE = "https://www.prada.com"
HUB_CID = "10628EU"
LANDING_URL = f"{BASE}/gb/en/fine-jewelry/categories/c/{HUB_CID}"
HUB_URL = LANDING_URL
...
LEAVES: list[tuple[str, str, str]] = [
    ("pr-fine-jewelry-bracelets", "Bracelets", "10629EU"),
    ("pr-fine-jewelry-necklaces", "Necklaces", "10630EU"),
    ("pr-fine-jewelry-rings", "Rings", "10631EU"),
    ("pr-fine-jewelry-earrings-brooches", "Earrings and brooches", "10632EU"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid in LEAVES}

BC_TO_LEAF = {
    "Bracelets": "pr-fine-jewelry-bracelets",
    "Necklaces": "pr-fine-jewelry-necklaces",
    "Rings": "pr-fine-jewelry-rings",
    "Earrings and brooches": "pr-fine-jewelry-earrings-brooches",
    "Pop Charms": "pr-fine-jewelry-earrings-brooches",
    "Bow": "pr-fine-jewelry-earrings-brooches",
}
LEAF_URL = {
    "pr-fine-jewelry-bracelets": f"{BASE}/gb/en/fine-jewelry/categories/bracelets/c/10629EU",
    "pr-fine-jewelry-necklaces": f"{BASE}/gb/en/fine-jewelry/categories/necklaces/c/10630EU",
    "pr-fine-jewelry-rings": f"{BASE}/gb/en/fine-jewelry/categories/rings/c/10631EU",
    "pr-fine-jewelry-earrings-brooches": (
        f"{BASE}/gb/en/fine-jewelry/categories/earrings-and-brooches/c/10632EU"
    ),
}

PARENT_COLS = ["prada", "prada-accessories", "pr-fine-jewelry"]
...
def fetch_hub_hits(s: cffi_requests.Session) -> list[dict]:
    """Union fine-jewelry hub + official leaf CIDs (dedupe by objectID)."""
    by_id: dict[str, dict] = {}
...
def infer_leaf_from_name(name: str) -> str | None:
    n = (name or "").lower()
    if any(x in n for x in ("bracelet", "bangle", "chain", "charm")):
        return "pr-fine-jewelry-bracelets"
    if any(x in n for x in ("necklace", "pendant", "choker")):
        return "pr-fine-jewelry-necklaces"
    if "ring" in n:
        return "pr-fine-jewelry-rings"
    if any(x in n for x in ("earring", "brooch", "cufflink", "stud", "hoop")):
        return "pr-fine-jewelry-earrings-brooches"
    return None
...
def collections_for(hit: dict) -> list[str]:
    ...
    for cid in cats:
        ...
        if lid:
            cols.add(lid)
    if not any(c.startswith("pr-fine-jewelry-") for c in cols):
    ...
    if not any(c.startswith("pr-fine-jewelry-") for c in cols):
        ...
        cols.add(inferred or "pr-fine-jewelry-rings")
    return sorted(cols)
...
def primary_leaf(cols: list[str], hit: dict) -> str:
    ...
    if mapped:
        return mapped
    name = ((hit.get("ProductName") or {}).get("en_GB") or "")
    return infer_leaf_from_name(name) or "pr-fine-jewelry-rings"
...
def parse_pdp(html: str, sku: str) -> dict:
    ...
    if tm:
        ...
        title = re.sub(
            r"^(Black|White|Forest|Beige|Brown|Blue|Red|Pink|Green|Grey|Gray|Navy|Ivory|Camel|Silver|Gold|Yellow)\s+",
            "",
            title,
            flags=re.I,
        ).strip()
...
def main() -> None:
    ...
    print("=== Prada Fine Jewelry (hub + leaves)", flush=True)
    ...
    PDP_CACHE.write_text(json.dumps(cache, ensure_ascii=False))
    no_price = sum(1 for p in products if p.get("gbpPrice") is None)
    print(f"Wrote {OUT_RAW} ({len(products)} products, {no_price} without GBP price)", flush=True)
