#!/usr/bin/env python3
"""Scrape Burberry Children PLPs + PDPs and merge into bb-catalog-raw.json."""
...
def main() -> None:
    ...
    for pid, prod in old_products.items():
        cols = [c for c in (prod.get("collections") or []) if not str(c).startswith(PREFIX)]
        if cols:
    ...
    for coll_id, label, path, top, _parent in CHILDREN_COLLECTIONS:
    ...
    print(f"Kids-linked colourways to enrich: {len(kids_ids)}", flush=True)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        futs = {ex.submit(job, pid): pid for pid in kids_ids}
        for fut in as_completed(futs):
            pid, pdp, err = fut.result()
            done += 1
            if done % 25 == 0 or done == len(kids_ids):
                PDP_CACHE.write_text(json.dumps(cache, ensure_ascii=False))
                print(f"  Kids PDP {done}/{len(kids_ids)}", flush=True)
            if err:
                errors.append({"id": pid, "error": err})

    extra_ids: list[str] = []
    for pid in list(kids_ids):
    ...
    if extra_ids:
        print(f"Kids sibling colourways: {len(extra_ids)}", flush=True)
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
    ...
    PDP_CACHE.write_text(json.dumps(cache, ensure_ascii=False))
    print(
        f"Wrote {RAW_PATH} products={len(products)} kids_errors={len(errors)}",
        flush=True,
    )
