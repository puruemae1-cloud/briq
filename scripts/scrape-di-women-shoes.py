#!/usr/bin/env python3
"""Scrape Dior GB Women's Shoes via Playwright WebKit.

Official leaves under Shoes → Dior → 여성용 (~402 SKUs from all-shoes). Run in 3
stages with a pause between for machine stability:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \
    python3 scripts/scrape-di-women-shoes.py --stage 1
  # pause ~5 min
  python3 scripts/scrape-di-women-shoes.py --stage 2
  # pause ~5 min
  python3 scripts/scrape-di-women-shoes.py --stage 3
...
OUT_RAW = ROOT / "src/data/di/di-women-shoes-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-women-shoes-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-women-shoes-pdp-cache.json"

HUB = f"{BASE}/{LANG}/fashion/womens-fashion/shoes/all-shoes"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_WOMEN_SHOES, leaf["id"]],
...
def main() -> None:
    ...
    if args.stage == "all":
        leaves = list(WOMEN_SHOES_LEAVES)
    else:
        leaves = [
            L
            for L in WOMEN_SHOES_LEAVES
            if L["id"] == args.stage or L.get("stage") == str(args.stage)
        ]
        if not leaves:
            raise SystemExit(f"unknown stage/leaf: {args.stage}")

    save_json(OUT_LEAVES, WOMEN_SHOES_LEAVES)
        ...
        for leaf in leaves:
            ...
            for i, hit in enumerate(hits, start=1):
                ...
                if prev:
                    ...
                    if leaf["id"] in ("di-women-shoes-all",) and prev.get("leafId"):
                        pl = prev["leafId"]
                        if pl not in ("di-women-shoes-all",):
                            row["leafId"] = pl
