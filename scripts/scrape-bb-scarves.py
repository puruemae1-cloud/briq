#!/usr/bin/env python3
"""Scrape Burberry Scarves hub PLPs and merge bb-scarves-* into bb-catalog-raw.json."""
...
def main() -> None:
    ...
    for coll_id, label, path, top, _parent in SCARVES_COLLECTIONS:
    ...
    print(
        f"Scarves-linked colourways: {len(scarf_ids)} (enrich {len(need)})",
        flush=True,
    )
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            ...
            for fut in as_completed(futs):
                ...
                if done % 25 == 0 or done == len(need):
                    ...
                    print(f"  Scarves PDP {done}/{len(need)}", flush=True)
                if err:
    ...
    PDP_CACHE.write_text(json.dumps(cache, ensure_ascii=False))
    print(
        f"Wrote {RAW_PATH} products={len(products)} scarves_errors={len(errors)}",
        flush=True,
    )
