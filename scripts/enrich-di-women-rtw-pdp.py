#!/usr/bin/env python3
"""Enrich Dior women's RTW catalog: Algolia variants, size charts, rich PDP copy.
...
Checkpoints every 25 SKUs so chat crashes do not lose progress.
  ...
  python3 scripts/enrich-di-women-rtw-pdp.py
  python3 scripts/enrich-di-women-rtw-pdp.py --only 013C501A4743_C080
  python3 scripts/enrich-di-women-rtw-pdp.py --translate   # slow: live translate missing
"""
...
from di_size_charts import size_chart_for_di_womens_rtw  # noqa: E402
...
RAW = ROOT / "src/data/di/di-women-rtw-catalog-raw.json"
...
def pick_leaf(product: dict, raw_row: dict | None) -> str:
    ...
    for leaf in (
        ...
        if leaf in cols:
            return leaf
    if raw_row:
        return raw_row.get("leafId") or "di-women-rtw-all"
    return product.get("subcategory") or "di-women-rtw-all"
