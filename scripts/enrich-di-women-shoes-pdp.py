#!/usr/bin/env python3
"""Enrich Dior women's shoes: Algolia size variants, official shoe size chart, rich KO PDP.

  python3 scripts/enrich-di-women-shoes-pdp.py
  python3 scripts/enrich-di-women-shoes-pdp.py --only KCV661OEN_S900
  python3 scripts/enrich-di-women-shoes-pdp.py --translate
"""
...
from di_common import WOMEN_SHOES_LEAVES, algolia_merch_hits_by_codes, gbp_to_krw  # noqa: E402
from di_size_charts import size_chart_for_di_womens_shoes  # noqa: E402
from ko_qa import is_good_korean  # noqa: E402

CAT = ROOT / "src/data/di/di-catalog.json"
PDP_CACHE = ROOT / "src/data/di/di-women-shoes-pdp-cache.json"
RAW = ROOT / "src/data/di/di-women-shoes-catalog-raw.json"
CHECKPOINT_EVERY = 25

SHOE_LEAVES = {
    "dior-shoes",
    "di-women-shoes",
    *[L["id"] for L in WOMEN_SHOES_LEAVES],
}
...
def pick_leaf(product: dict, raw_row: dict) -> str:
    ...
    for leaf_id in (L["id"] for L in WOMEN_SHOES_LEAVES if L["id"] != "di-women-shoes-all"):
        if leaf_id in cols:
            return leaf_id
    raw_leaf = (raw_row or {}).get("leafId") or ""
    if raw_leaf in SHOE_LEAVES and raw_leaf != "di-women-shoes-all":
        return raw_leaf
    if "di-women-shoes-all" in cols or product.get("subcategory") == "di-women-shoes-all":
        return "di-women-shoes-all"
    return str(product.get("subcategory") or "di-women-shoes-all")


def story_sections_for_shoes(
    description_ko: str,
    images: list[str],
    *,
    ...
    detail_body = (
        " · ".join(features_ko[:8])
        if features_ko
        else "Dior 공식 제품 컷으로 확인하는 실루엣·굽 높이·시그니처 디테일입니다."
    )
    material_body = (
        ...
        sections.append(
            {
                "titleKo": "디올 룩",
                "bodyKo": (
                    "Dior 여성 컬렉션 룩과 함께 제안되는 스타일링 레퍼런스입니다. "
                    "공식 룩북 컷으로 핏과 무드를 확인해 보세요."
                ),
...
def enrich_one(
    ...
    *,
    ...
    product["storySections"] = story_sections_for_shoes(
        description_ko,
        images,
    ...
    if "di-women-shoes" not in collections:
        collections.append("di-women-shoes")
