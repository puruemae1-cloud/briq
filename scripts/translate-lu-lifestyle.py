#!/usr/bin/env python3
"""Translate London Undercover lifestyle PDP copy into lu-translate-cache.json."""
    ...
    for p in raw.values():
        ...
        for v in p.get("variants") or []:
            for key in ("option1", "option2", "title"):
                c = (v.get(key) or "").strip()
                if c and c.lower() not in ("default title", "default"):
                    out.add(c)
    ...
    for s in todo:
        ...
        if ko:
            ...
            if done <= 8 or done % 25 == 0:
                print(f"[{done}/{len(todo)}] {s[:65]} => {ko[:65]}")
        if done % 20 == 0:
