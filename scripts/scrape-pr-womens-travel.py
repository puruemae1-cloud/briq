#!/usr/bin/env python3
"""Scrape Prada GB women's travel (~32) + PDP images.

Source hub: https://www.prada.com/gb/en/womens/travel/c/10094EU
PLP membership uses Algolia CategoriesEnriched ``10094EU|false|false``.
Official leaves under Travel: travel bags, luggage & carry-on, travel accessories.
"""
...
OUT_RAW = ROOT / "src/data/pr/pr-womens-travel-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-womens-travel-pdp-cache.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"

BASE = "https://www.prada.com"
HUB_URL = f"{BASE}/gb/en/womens/travel/c/10094EU"
HUB_CID = "10094EU"
...
# Official leaf PLPs under women's travel (prada.com GB hub menu)
LEAVES: list[tuple[str, str, str]] = [
    ("pr-women-travel-bags", "Travel bags", "10099EU"),
    ("pr-women-luggage-carry-on", "Luggage and carry-on", "10097EU"),
    ("pr-women-travel-accessories", "Travel accessories", "11064EU"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid in LEAVES}
BC_TO_LEAF = {
    "Travel bags": "pr-women-travel-bags",
    "Luggage and carry-on": "pr-women-luggage-carry-on",
    "Travel accessories": "pr-women-travel-accessories",
    "Pouches": "pr-women-travel-accessories",
    "Trolleys": "pr-women-luggage-carry-on",
    "Duffle bags": "pr-women-travel-bags",
    "Duffel bags": "pr-women-travel-bags",
}
LEAF_SLUG = {
    "pr-women-travel-bags": "travel-bags",
    "pr-women-luggage-carry-on": "luggage-and-carry-on",
    "pr-women-travel-accessories": "travel-accessories",
}

PARENT_COLS = ["prada", "prada-bags", "pr-women-travel"]
MAX_WORKERS = 6
IMG_WORKERS = 8
MAX_IMAGES = 12
...
def infer_leaf_from_name(name: str) -> str | None:
    n = (name or "").lower()
    if any(x in n for x in ("trolley", "suitcase", "cabin", "carry-on", "carry on")):
        return "pr-women-luggage-carry-on"
    if any(x in n for x in ("pouch", "cosmetic", "toiletry", "organizer")):
        return "pr-women-travel-accessories"
    if any(x in n for x in ("duffel", "duffle", "travel bag", "weekender")):
        return "pr-women-travel-bags"
    return None
...
def primary_leaf(cols: list[str], hit: dict) -> str:
    ...
    for lid, _label, _cid in LEAVES:
        if lid in cols:
            return lid
    name = ((hit.get("ProductName") or {}).get("en_GB") or "")
    return infer_leaf_from_name(name) or "pr-women-travel-bags"
...
def main() -> None:
    ...
    print("=== Prada women's travel hub (Algolia)", flush=True)
    ...
    for lid, label, cid in LEAVES:
        ...
        plp_meta[lid] = {
            ...
            "categoryCode": cid,
            "count": n,
            "url": f"{BASE}/gb/en/womens/travel/{LEAF_SLUG[lid]}/c/{cid}",
        }
