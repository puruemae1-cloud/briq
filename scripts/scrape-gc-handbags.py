#!/usr/bin/env python3
"""Scrape Gucci UK women's handbags (8 official subcategories) + PDP images."""
...
# Official handbag leaves (Gucci UK nav) → Briq collection ids
COLLECTIONS: list[tuple[str, str, str]] = [
    ("gc-women-shoulder-bags", "Shoulder Bags", "women-handbags-shoulder-bags"),
    ("gc-women-mini-bags", "Mini Bags", "women-handbags-mini-bags"),
    ("gc-women-crossbody-bags", "Crossbody Bags", "women-handbags-crossbody-bags"),
    ("gc-women-tote-bags", "Tote Bags", "women-handbags-totes"),
    ("gc-women-top-handle-bags", "Top Handle Bags", "women-handbags-top-handles-and-boston-bags"),
    ("gc-women-backpacks-beltbags", "Backpacks & Beltbags", "women-handbags-backpacks"),
    ("gc-women-clutches-evening", "Clutches & Evening Bags", "women-handbags-clutches-and-evening"),
    ("gc-women-personalised", "Personalised Handbags", "monogramming-handbags"),
]

ALL_HANDBAGS_CODE = "women-handbags"
MAX_WORKERS = 6
IMG_WORKERS = 8
...
def extract_shot_key(u: str) -> str:
    """Dedupe by style shot id (e.g. 837466_AAEAO_2718_001)."""
    fn = u.split("/")[-1]
...
def fetch_pdp_images(s: cffi_requests.Session, product_link: str) -> list[str]:
    ...
    seen: set[str] = set()
    for u in found:
        # skip colour swatches / tiny thumbs
        if "_150x150/" in u or "_600x314/" in u:
            continue
        uu = upgrade_media_url(u)
        k = extract_shot_key(uu)
        # only keep shots for this colourway folder naming — caller filters further
        if k in seen:
            continue
        seen.add(k)
        out.append(uu)
...
def main() -> None:
    ...
    # Warm cookies
    s.get(
        f"{BASE}/uk/en_gb/ca/women/handbags-c-women-handbags",
    ...
    # Parent all-handbags for complete set
    print("=== women-handbags (all)", flush=True)
    all_items = fetch_grid_category(s, ALL_HANDBAGS_CODE)
    plp_meta["gc-handbags"] = {
        "label": "Handbags",
        "categoryCode": ALL_HANDBAGS_CODE,
        "count": len({it["productCode"] for it in all_items}),
    }
    for it in all_items:
        code = it["productCode"]
        membership.setdefault(code, set()).add("gc-handbags")
        cards[code] = it
    ...
    for coll_id, label, cat_code in COLLECTIONS:
        ...
        for it in items:
            code = it["productCode"]
            membership.setdefault(code, set()).add(coll_id)
            # Prefer richer card if missing
            if code not in cards:
                cards[code] = it
            else:
                # merge images if more available
                prev = cards[code]
    ...
    def enrich(code: str) -> dict:
        ...
        if not pdp_imgs:
            ...
            # productCode like 837466AAEAO2718 → file 837466_AAEAO_2718
        ...
        # Merge: PDP first (higher quality order), then grid
        images: list[str] = []
        seen: set[str] = set()
    ...
    def dl_one(prod: dict) -> tuple[str, list[str]]:
        ...
        for i, url in enumerate(prod.get("images") or [], start=1):
            ...
            ok = download_image(local_s, url, dest)
            if ok:
    ...
    payload = {
        "scrapedAt": datetime.now(timezone.utc).isoformat(),
        "source": f"{BASE}/uk/en_gb/ca/women/handbags-c-women-handbags",
