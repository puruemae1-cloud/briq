#!/usr/bin/env python3
"""Scrape Gucci UK men's ready-to-wear (Clothing) + PDP images.

Mirrors scrape-gc-womens-rtw.py: productgrid leaves + catalog API + DarkGray
images into public/products/gc-pdp/. Writes gc-mens-rtw-catalog-raw.json.
"""
...
OUT_RAW = ROOT / "src/data/gc/gc-mens-rtw-catalog-raw.json"
...
WARM_URL = f"{BASE}/uk/en_gb/ca/men/ready-to-wear-for-men-c-men-readytowear"

# Official Clothing leaves (Gucci UK men RTW nav) → Briq collection ids
# categoryCode is the URL segment after -c-
COLLECTIONS: list[tuple[str, str, str]] = [
    (
        "gc-men-tshirts-polos",
        "T-Shirts & Polo Shirts",
        "men-readytowear-tshirts-and-polos",
    ),
    (
        "gc-men-tracksuit-sweatshirts",
        "Tracksuit & Sweatshirts",
        "men-ready-to-wear-new-sweatshirts",
    ),
    ("gc-men-shirts", "Shirts", "men-readytowear-shirts"),
    ("gc-men-knitwear", "Knitwear", "men-readytowear-sweaters-cardigans"),
    ("gc-men-denim", "Denim", "men-readytowear-denim"),
    (
        "gc-men-trousers-shorts",
        "Trousers & Shorts",
        "men-readytowear-trousers-and-shorts",
    ),
    ("gc-men-swimwear", "Swimwear", "men-swimwear"),
    ("gc-men-outerwear", "Outerwear", "men-readytowear-outerwear"),
    ("gc-men-leather", "Leather", "men-leather"),
    ("gc-men-formal-wear", "Formal Wear", "jackets-men"),
    ("gc-men-coats-jackets", "Coats & Jackets", "men-coats-jackets"),
]

ALL_RTW_CODE = "men-readytowear"
PARENT_COLL = "gc-men-rtw"
MAX_WORKERS = 6
IMG_WORKERS = 8
...
def main() -> None:
    ...
    for it in all_items:
        code = it["productCode"]
        membership.setdefault(code, set()).update({PARENT_COLL, "gc-men", "gucci"})
        cards[code] = it
    ...
    for coll_id, label, cat_code in COLLECTIONS:
        ...
        for it in items:
            ...
            membership.setdefault(code, set()).update(
                {coll_id, PARENT_COLL, "gc-men", "gucci"}
            )
