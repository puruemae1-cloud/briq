#!/usr/bin/env python3
"""Enrich Arc'teryx outdoor gear (accessories/packs/climbing) PDPs via Playwright (__NEXT_DATA__).

Captures per colour×size stockStatus, full galleries, description/features,
then downloads galleries into public/products/axg-pdp.

Usage:
  python3 scripts/enrich-ax-gear-playwright.py
  python3 scripts/enrich-ax-gear-playwright.py --limit 5
"""
...
def fetch_one(page, url: str) -> dict:
    ...
    try:
        ...
        return {"error": f"evaluate: {e}", "url": url}
    if not next_data:
        return {"error": "no __NEXT_DATA__", "url": url}
    try:
        raw_prod = next_data["props"]["pageProps"].get("product")
    except Exception:
        return {"error": "bad next_data shape", "url": url}
    if not raw_prod:
        return {"error": "no product", "url": url}
...
def main() -> None:
    ...
    with sync_playwright() as pw:
        ...
        for i, p in enumerate(products):
            ...
            time.sleep(0.5 if refresh else 2.0)
