#!/usr/bin/env python3
"""Scrape Dior GB Men's Accessories via Playwright WebKit.

Official leaves under Accessories → Dior → 남성용 (~377 SKUs from all-accessories). Run in 3
stages with a pause between for machine stability:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \\
    python3 scripts/scrape-di-men-accessories.py --stage 1
  # pause ~5 min
  python3 scripts/scrape-di-men-accessories.py --stage 2
  # pause ~5 min
  python3 scripts/scrape-di-men-accessories.py --stage 3
  python3 scripts/merge-di-catalog-ko.py
  python3 scripts/enrich-di-men-accessories-pdp.py
...
OUT_RAW = ROOT / "src/data/di/di-men-accessories-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-men-accessories-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-men-accessories-pdp-cache.json"

HUB = f"{BASE}/{LANG}/fashion/mens-fashion/accessories/all-accessories"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_MEN_ACCESSORIES, leaf["id"]],
...
def main() -> None:
    ...
    if args.stage == "all":
        leaves = list(MEN_ACCESSORIES_LEAVES)
    else:
        leaves = [
            L
            for L in MEN_ACCESSORIES_LEAVES
            if L["id"] == args.stage or L.get("stage") == str(args.stage)
        ]
        if not leaves:
            raise SystemExit(f"unknown stage/leaf: {args.stage}")

    save_json(OUT_LEAVES, MEN_ACCESSORIES_LEAVES)
        ...
        for leaf in leaves:
            ...
            for i, hit in enumerate(hits, start=1):
                ...
                if prev:
                    ...
                    if leaf["id"] in ("di-men-acc-all",) and prev.get("leafId"):
                        pl = prev["leafId"]
                        if pl not in ("di-men-acc-all",):
                            row["leafId"] = pl
