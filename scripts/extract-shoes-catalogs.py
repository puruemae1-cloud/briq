#!/usr/bin/env python3
"""Extract shoes-only catalogue slices from full brand JSON.
...
EXPORT_NAME = {
    "bb": "bbShoesCatalogProducts",
    "ax": "axShoesCatalogProducts",
    "ps": "psShoesCatalogProducts",
    "bs": "bsShoesCatalogProducts",
    "gc": "gcShoesCatalogProducts",
    "bv": "bvShoesCatalogProducts",
    "ch": "chShoesCatalogProducts",
    "al": "alShoesCatalogProducts",
    "ce": "ceShoesCatalogProducts",
    "vw": "vwShoesCatalogProducts",
    "pr": "prShoesCatalogProducts",
    "di": "diShoesCatalogProducts",
    "mb": "mbShoesCatalogProducts",
    "ys": "ysShoesCatalogProducts",
}
...
def main() -> None:
    ...
    for key, paths in SOURCES.items():
        ...
        for p in shoes:
            ...
            seen.add(pid)
            uniq.append(p)
        out_json = DATA / key / f"{key}-shoes-catalog.json"
        out_ts = DATA / key / f"{key}-shoes-catalog.ts"
        ...
        out_ts.write_text(
            ...
            f'import data from "./{key}-shoes-catalog.json";\n\n'
            f"/** Shoes-only slice — shoes PLPs must not parse the full {key} catalogue. */\n"
            f"export const {export} = data as unknown as Product[];\n"
        )
        total_n += len(uniq)
        total_bytes += out_json.stat().st_size
        print(f"{key}: {len(uniq)} shoes → {out_json.stat().st_size / 1e6:.2f}MB")
