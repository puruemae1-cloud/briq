...
def main() -> None:
    ...
    OUT_TS.write_text(
        ...
        f"export const vwCatalogProducts = {json.dumps(products, ensure_ascii=False)} as Product[];\n"
        f"export const vwCatalogGeneratedAt = {json.dumps(datetime.now(timezone.utc).isoformat())};\n"
    )
    print(f"wrote {len(products)} products -> {OUT_TS}", flush=True)
