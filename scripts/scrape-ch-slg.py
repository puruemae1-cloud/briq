#!/usr/bin/env python3
"""Scrape Chanel GB small leather goods into ch-slg-catalog-raw.json + images.

Official leaves (SLG hub https://www.chanel.com/gb/fashion/small-leather-goods/c/1x1x2/):
  Wallets on Chain / Micro Bags / Vanity /
  Card Holders & Wallets / Pouches & Cases / Leather Accessories

Reuses ChanelClient + image helpers from scrape-ch-rtw.py (Akamai/proxy).
SKU prefixes: AP* (small leather goods; not handbags A*/AS*, jewellery AB*, shoes G*).
"""
...
OUT_RAW = ROOT / "src/data/ch/ch-slg-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/ch/ch-slg-pdp-cache.json"
...
LEAVES: list[tuple[str, str, str, str]] = [
    (
        ...
        "Wallets on Chain",
        "월렛 온 체인",
        f"{BASE}/gb/fashion/small-leather-goods/c/1x1x2x15/wallets-on-chain/",
    ),
    (
        "ch-women-micro-bags",
        "Micro Bags",
        "마이크로백",
        f"{BASE}/gb/fashion/small-leather-goods/c/1x1x2x26/micro-bags/",
    ),
    (
        "ch-women-vanity",
        "Vanity",
        "배니티",
        f"{BASE}/gb/fashion/small-leather-goods/c/1x1x2x24/vanity/",
    ),
    (
        "ch-women-card-holders-wallets",
        "Card Holders & Wallets",
        "카드홀더 & 월렛",
        # Official hub filter unions Card Holders + Long Wallets.
        # GB card-holders PLP intermittently 404s; long-wallets is stable and
        # card-holder PDPs are also recovered via hub SKUs + hierarchy.
        f"{BASE}/gb/fashion/small-leather-goods/c/1x1x2x5/long-wallets/",
    ),
    (
        "ch-women-pouches-cases",
        "Pouches & Cases",
        "파우치 & 케이스",
        f"{BASE}/gb/fashion/small-leather-goods/c/1x1x2x4/pouches-cases/",
    ),
    (
        "ch-women-leather-accessories",
        "Leather Accessories",
        "레더 액세서리",
        f"{BASE}/gb/fashion/small-leather-goods/c/1x1x2x25/leather-accessories/",
    ),
]

# Extra PLP URLs that map into an existing leaf (hub filter unions).
EXTRA_LEAF_PLPS: list[tuple[str, str]] = [
    (
        "ch-women-card-holders-wallets",
        f"{BASE}/gb/fashion/small-leather-goods/c/1x1x2x1/card-holders/",
    ),
]
...
LEAF_BY_SLUG = {
    "wallets-on-chain": "ch-women-wallets-on-chain",
    "micro-bags": "ch-women-micro-bags",
    "vanity": "ch-women-vanity",
    "card-holders": "ch-women-card-holders-wallets",
    "long-wallets": "ch-women-card-holders-wallets",
    "pouches-cases": "ch-women-pouches-cases",
    "leather-accessories": "ch-women-leather-accessories",
}
...
def order_slg_images(images: list[dict]) -> list[str]:
...
order_images = order_slg_images


def is_slg_sku(sku: str) -> bool:
    s = (sku or "").upper()
    return bool(re.fullmatch(r"AP[A-Z0-9]+", s))
...
def extract_skus_from_html(html: str) -> set[str]:
    ...
    return {s for s in skus if is_slg_sku(s)}
...
def discover_sitemap_slg_skus(client: ChanelClient) -> dict[str, str]:
    ...
    for m in re.finditer(
        r"https://www\.chanel\.com/gb/fashion/p/(AP[^/\s\"<]+)/",
        text,
        flags=re.I,
    ):
        sku = m.group(1)
        if not is_slg_sku(sku):
            continue
        by_sku.setdefault(sku, m.group(0).rstrip("/") + "/")
    log(f"sitemap AP* SKUs: {len(by_sku)}")
    return by_sku
    ...
    for h in prod.get("hierarchy") or []:
        ...
        for slug, cid in LEAF_BY_SLUG.items():
            if f"/{slug}/" in url or url.rstrip("/").endswith(f"/{slug}"):
                return cid
        if "/1x1x2x15/" in url or "wallet on chain" in label or "wallets on chain" in label:
            return "ch-women-wallets-on-chain"
        if "/1x1x2x26/" in url or "micro bag" in label:
            return "ch-women-micro-bags"
        if "/1x1x2x24/" in url or "vanity" in label:
            return "ch-women-vanity"
        if (
            "/1x1x2x1/" in url
            or "/1x1x2x5/" in url
            or "card holder" in label
            or "long wallet" in label
            or label.strip() in {"wallets", "wallet"}
        ):
            return "ch-women-card-holders-wallets"
        if "/1x1x2x4/" in url or "pouch" in label or "case" in label:
            return "ch-women-pouches-cases"
        if "/1x1x2x25/" in url or "leather accessor" in label:
            return "ch-women-leather-accessories"
    cat = (prod.get("categoryLabel") or "").lower()
    if "wallet on chain" in cat or "wallets on chain" in cat:
        return "ch-women-wallets-on-chain"
    if "micro bag" in cat:
        return "ch-women-micro-bags"
    if "vanity" in cat:
        return "ch-women-vanity"
    if "card holder" in cat or "wallet" in cat:
        return "ch-women-card-holders-wallets"
    if "pouch" in cat or "case" in cat:
        return "ch-women-pouches-cases"
    if "leather accessor" in cat:
        return "ch-women-leather-accessories"
    return None


def resolve_slg_leaf(prod: dict, forced_leaves: set[str] | None = None) -> str | None:
    """Prefer PDP hierarchy over PLP membership."""
...
def parse_slg_pdp(html: str, url: str, forced_leaves: set[str]) -> dict | None:
    ...
    if not sku or not is_slg_sku(sku):
        return {"_skip": True, "reason": f"not-slg-sku:{sku}", "sku": sku, "url": url}
    ...
    primary = resolve_slg_leaf(prod, forced_leaves)
    if not primary:
        return {
            "_skip": True,
            "sku": sku,
...
def main() -> int:
    ...
    _rtw.HUB = HUB
    _rtw.PROBE_SKU_URL = (
        "https://www.chanel.com/gb/fashion/p/AP4241Y01480C3906/"
        "classic-wallet-on-chain-lambskin/"
    )
    ...
    sitemap = discover_sitemap_slg_skus(client)
    hub_ids = discover_plp_skus(client, HUB)
    for sku in hub_ids:
        # Hub-only SKUs (e.g. card holders when PLP 404s) get leaf from PDP hierarchy.
        sku_urls.setdefault(sku, sitemap.get(sku) or f"{BASE}/gb/fashion/p/{sku}/")

    # Prefer leaf-tagged SKUs; keep hub discoveries for hierarchy classification.
    for sku, leaves in sku_leaves.items():
        sku_urls[sku] = sitemap.get(sku) or f"{BASE}/gb/fashion/p/{sku}/"

    todo = sorted(sku_urls.items())
    log(
        f"unique SLG SKUs to scrape: {len(todo)} "
    ...
    while i < len(todo):
        ...
        if (
            ...
            and cached.get("leaf")
            and cached.get("kind") == "slg"
            and not cached.get("_skip")
        ):
            primary = resolve_slg_leaf(cached, forced) or cached["leaf"]
            leaves = {primary}
            cached["leaf"] = primary
        ...
        consecutive_blocks = 0
        parsed = parse_slg_pdp(html, url, forced)
        if not parsed:
    ...
    payload = {
        ...
        "hub": HUB,
        "note": (
            "Chanel GB small leather goods by official PLP leaves; "
            "Card Holders & Wallets unions card-holders + long-wallets."
        ),
        "leaves": [
    ...
    log(f"wrote {len(products)} SLG → {OUT_RAW}")
