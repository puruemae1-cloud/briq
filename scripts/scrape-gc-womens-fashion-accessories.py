#!/usr/bin/env python3
"""Scrape Gucci UK women's fashion accessories (non-wallet) + PDP images.

Mirrors scrape-gc-jewellery-watches.py / scrape-gc-womens-shoes.py: productgrid
leaves + catalog API sizes + DarkGray images into public/products/gc-pdp/.
Writes gc-fashion-accessories-catalog-raw.json.

Official hub:
https://www.gucci.com/uk/en_gb/ca/women/accessories-for-women-c-women-accessories

Nav leaves (Briq): belts, scarves/silks, hats/gloves, eyewear, hair, socks/tights.
Bag charms PLP is scraped too (reuse gc-women-bag-charms-keychains) — usually
already imported with wallets; build-gc-catalog skips exact duplicates.
"""
...
OUT_RAW = ROOT / "src/data/gc/gc-fashion-accessories-catalog-raw.json"
...
WARM_URL = f"{BASE}/uk/en_gb/ca/women/accessories-for-women-c-women-accessories"
...
COLLECTIONS: list[tuple[str, str, str]] = [
    ("gc-women-belts", "Belts", "women-accessories-belts"),
    (
        "gc-women-scarves-silks",
        "Scarves & Silks",
        "women-accessories-silks-and-scarves",
    ),
    (
        "gc-women-hats-gloves",
        "Hats & Gloves",
        "women-accessories-hats-and-gloves",
    ),
    ("gc-women-eyewear", "Eyewear", "women-eyewear"),
    (
    ...
    (
        ...
        "Socks & Tights",
        "women-accessories-socks-tights",
    ),
]

# Soft-accessory parent (excludes wallets / travel / jewellery hubs)
ALL_FASHION_CODE = "women-accessories"
PARENT_COLL = "gc-women-fashion-accessories"
PARENT_COLLS = (
    "gc-women-fashion-accessories",
    "gucci-accessories",
)

# Bag charms already live under wallets nav — keep wallet parents for membership.
BAG_CHARMS_ID = "gc-women-bag-charms-keychains"
BAG_CHARMS_EXTRA = (
    "gc-women-wallets",
...
def main() -> None:
    ...
    # Leaf PLPs only — hub "women-accessories" also includes wallets/jewellery.
