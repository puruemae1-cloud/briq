...
Men's RTW / men's bags / men's shoes / men's wallets / men's fashion
accessories / men's travel / men's gifts: exact duplicate productCodes tag
...
MENS_FASHION_ACC_RAW = ROOT / "src/data/gc/gc-mens-fashion-accessories-catalog-raw.json"
TRAVEL_RAW = ROOT / "src/data/gc/gc-travel-catalog-raw.json"
MENS_TRAVEL_RAW = ROOT / "src/data/gc/gc-mens-travel-catalog-raw.json"
JEWELLERY_RAW = ROOT / "src/data/gc/gc-jewellery-catalog-raw.json"
GIFTS_RAW = ROOT / "src/data/gc/gc-gifts-catalog-raw.json"
MENS_GIFTS_RAW = ROOT / "src/data/gc/gc-mens-gifts-catalog-raw.json"
...
# Men's soft accessories (belts/eyewear/hats/ties/scarves/socks/charms).
# Charms may overlap gc-men-keyrings-keycases — duplicates tag membership.
MEN_FASHION_ACC_LEAF_COLLECTIONS = [
    "gc-men-belts",
    "gc-men-eyewear",
    "gc-men-hats-gloves",
    "gc-men-ties",
...
MEN_FASHION_ACC_PARENT_COLLECTIONS = [
...
def fashion_acc_size_kind(row: dict, size_rows: list[dict]) -> str:
    ...
    if not meaningful:
        return "onesize"
    if "gc-women-belts" in cols or "gc-men-belts" in cols:
        return "belt"
    if "gc-women-hats-gloves" in cols or "gc-men-hats-gloves" in cols:
        return "hat"
    if "gc-women-socks-tights" in cols or "gc-men-socks" in cols:
        return "socks"
...
def build_mens_fashion_accessory_product(
    row: dict, prev: dict | None, now_iso: str
) -> dict | None:
    """Men's belts / eyewear / hats / ties / scarves / socks / charms."""
    code = row.get("productCode") or row.get("id")
    if not code:
        return None
    gbp = row.get("gbpPrice")
    ...
    allowed = {
        *MEN_FASHION_ACC_LEAF_COLLECTIONS,
        *MEN_FASHION_ACC_PARENT_COLLECTIONS,
        # Charms may also carry wallet parents from the scraper.
        *MEN_WALLETS_PARENT_COLLECTIONS,
        "gc-men-wallets",
    }
    cols = [c for c in (row.get("collections") or []) if c in allowed]
    cols = sorted(set([*cols, *MEN_FASHION_ACC_PARENT_COLLECTIONS]))

    leaf = next(
        (c for c in MEN_FASHION_ACC_LEAF_COLLECTIONS if c in cols),
        "gc-men-fashion-accessories",
    )
    copy = common_copy(row)
    if not copy:
...
def build_travel_product(row: dict, prev: dict | None, now_iso: str) -> dict | None:
    """Women's travel bags / luggage — One Size; W×H×D in PDP detail copy."""
    code = row.get("productCode") or row.get("id")
    if not code:
        return None
    gbp = row.get("gbpPrice")
...
def build_mens_travel_product(
    row: dict, prev: dict | None, now_iso: str
) -> dict | None:
    """Men's travel bags / luggage — One Size; W×H×D in PDP detail copy."""
    code = row.get("productCode") or row.get("id")
    if not code:
        return None
    gbp = row.get("gbpPrice")
    ...
    allowed = {*MEN_TRAVEL_LEAF_COLLECTIONS, *MEN_TRAVEL_PARENT_COLLECTIONS}
    cols = [c for c in (row.get("collections") or []) if c in allowed]
    cols = sorted(set([*cols, *MEN_TRAVEL_PARENT_COLLECTIONS]))
...
def build_gift_product(row: dict, prev: dict | None, now_iso: str) -> dict | None:
    ...
    is_mens = (row.get("kind") or "").lower() in {
        "mens-gifts",
        "mens_gifts",
        "men-gifts",
    }
    if is_mens:
        ...
        cols = [c for c in (row.get("collections") or []) if c in allowed]
        cols = sorted(set([*cols, *MEN_GIFTS_PARENT_COLLECTIONS]))
        leaf = next(
            (c for c in MEN_GIFTS_LEAF_COLLECTIONS if c in cols), "gc-men-gifts"
        )
    else:
    ...
    if "gc-gifts-personalised" in cols or "gc-men-gifts-personalised" in cols:
    ...
    size_chart = None
    if "gc-men-gifts-shoes" in cols:
        size_chart = GC_MEN_SHOES_SIZE_CHART
    elif "gc-men-gifts-belts" in cols:
        size_chart = GC_BELT_SIZE_CHART
    elif "gc-men-gifts-jewellery-watches" in cols or "gc-men-gifts-watches" in cols:
        # Prefer ring chart when sized like rings; otherwise bracelet if numeric.
        size_labels = [
            str(sz.get("size") or "").strip().upper()
            for sz in size_rows
            if str(sz.get("size") or "").strip()
        ]
        if any(re.search(r"\b(RING|RG)\b", s) or re.fullmatch(r"\d{1,2}", s) for s in size_labels):
            size_chart = GC_RING_SIZE_CHART
        elif any("BRACELET" in s or "BR" in s for s in size_labels):
            size_chart = GC_BRACELET_SIZE_CHART
...
def build_product(row: dict, prev: dict | None, now_iso: str) -> dict | None:
    ...
    if kind in {"gifts", "mens-gifts", "mens_gifts", "men-gifts"}:
    ...
    if kind in {"mens-travel", "mens_travel", "men-travel"} or any(
        c in MEN_TRAVEL_LEAF_COLLECTIONS or c == "gc-men-travel" for c in cols
    ):
        return build_mens_travel_product(row, prev, now_iso)
    ...
    if kind in {
        ...
        "mens_fashion_accessories",
        "men-fashion-accessories",
    } or any(
        c in MEN_FASHION_ACC_LEAF_COLLECTIONS or c == "gc-men-fashion-accessories"
        for c in cols
    ):
        # Pure bag-charms rows (no other fashion leaf) that only carry wallet
        # parents still use the fashion builder — men's charms live under
        # 패션 액세서리 nav (unlike women's wallets leaf).
        return build_mens_fashion_accessory_product(row, prev, now_iso)
...
def dedupe_style_color_name(products: list[dict]) -> list[dict]:
    ...
    for group in buckets.values():
        ...
        winner = ranked[0]
        # Merge category membership from dropped clones (e.g. gifts-for-him
        # material codes that share style+colour with an existing belt PDP).
        for loser in ranked[1:]:
            extra = loser.get("gcCollections") or []
            if extra:
                winner["gcCollections"] = sorted(
                    set(winner.get("gcCollections") or []) | set(extra)
                )
                for v in winner.get("variants") or []:
                    if "gcCollections" in v:
                        v["gcCollections"] = sorted(
                            set(v.get("gcCollections") or []) | set(extra)
                        )
            tags = list(winner.get("tags") or [])
        ...
        for loser in ranked[1:]:
            print(
                f"dedupe drop {loser.get('id')} (keep {winner.get('id')})",
                flush=True,
            )
    if dropped:
...
def mens_gifts_cols_from_row(row: dict) -> list[str]:
    allowed = {*MEN_GIFTS_LEAF_COLLECTIONS, *MEN_GIFTS_PARENT_COLLECTIONS}
    cols = {c for c in (row.get("collections") or []) if c in allowed}
    if any(c in MEN_GIFTS_LEAF_COLLECTIONS for c in cols) or "gc-men-gifts" in cols:
        cols.add("gc-men-gifts")
        cols.add("gc-accessories-mens")
        cols.add("gucci-accessories")
    return sorted(cols)


def apply_mens_gifts_membership(
    products: list[dict], mens_tags: dict[str, list[str]]
) -> int:
    """Merge men's gifts-for-him collection ids onto existing PDPs.

    Exact duplicates already under gc-gifts* / other categories are tagged only.
    Also matches style+colour when the gifts SKU used a different material code.
    """
    by_style_color: dict[tuple[str, str], list[dict]] = {}
    for p in products:
        sc = style_color_key(str(p.get("sku") or ""))
        if sc:
            by_style_color.setdefault(sc, []).append(p)

    tagged = 0
    for sku, extra in mens_tags.items():
        if not extra:
            continue
        targets: list[dict] = []
        for p in products:
            if str(p.get("sku") or "").upper() == sku:
                targets.append(p)
        if not targets:
            sc = style_color_key(sku)
            if sc:
                candidates = list(by_style_color.get(sc) or [])
                if candidates:
                    # One stand-in only — avoid tagging every material twin.
                    candidates.sort(
                        key=lambda p: (
                            len(p.get("images") or []),
                            len(p.get("variants") or []),
...
def mens_fashion_acc_cols_from_row(row: dict) -> list[str]:
    allowed = {
        *MEN_FASHION_ACC_LEAF_COLLECTIONS,
        *MEN_FASHION_ACC_PARENT_COLLECTIONS,
        *MEN_WALLETS_PARENT_COLLECTIONS,
    ...
    cols = {c for c in (row.get("collections") or []) if c in allowed}
    if any(c in MEN_FASHION_ACC_LEAF_COLLECTIONS for c in cols):
        cols.update(MEN_FASHION_ACC_PARENT_COLLECTIONS)
    elif "gc-men-fashion-accessories" in cols or "gc-accessories-mens" in cols:
        cols.update(MEN_FASHION_ACC_PARENT_COLLECTIONS)
    return sorted(cols)


def apply_mens_fashion_acc_membership(
    products: list[dict], mens_tags: dict[str, list[str]]
) -> int:
    """Merge men's fashion-accessory ids onto existing PDPs (esp. keyrings/charms).

    Exact duplicates are not re-imported; they still appear under 패션 액세서리.
    """
    tagged = 0
    for p in products:
        sku = str(p.get("sku") or "").upper()
        ...
        for c in extra:
            if c not in tags:
                tags.append(c)
        for t in ("accessories", "악세서리", "패션 액세서리", "남성", "mens"):
            if t not in tags:
                tags.append(t)
        p["tags"] = tags
        for v in p.get("variants") or []:
...
def mens_travel_cols_from_row(row: dict) -> list[str]:
    allowed = {*MEN_TRAVEL_LEAF_COLLECTIONS, *MEN_TRAVEL_PARENT_COLLECTIONS}
    cols = {c for c in (row.get("collections") or []) if c in allowed}
    if any(c in MEN_TRAVEL_LEAF_COLLECTIONS for c in cols):
        cols.add("gc-men-travel")
        cols.add("gc-accessories-mens")
        cols.add("gucci-accessories")
    elif "gc-men-travel" in cols or "gc-accessories-mens" in cols:
        cols.add("gc-men-travel")
        cols.add("gc-accessories-mens")
        cols.add("gucci-accessories")
    return sorted(cols)
...
def apply_mens_travel_membership(
    products: list[dict], mens_tags: dict[str, list[str]]
) -> int:
    """Merge men's travel collection ids onto existing PDPs (bags / women's travel).

    Exact duplicates (esp. duffles already in men's handbags) are not re-imported.
    """
    tagged = 0
    for p in products:
        sku = str(p.get("sku") or "").upper()
        ...
        for c in extra:
            if c not in tags:
                tags.append(c)
        for t in ("travel", "여행", "luggage", "러기지", "악세서리", "남성", "mens"):
            if t not in tags:
                tags.append(t)
        p["tags"] = tags
        for v in p.get("variants") or []:
...
def load_rows() -> tuple:
    ...
    mens_wallet_tags / mens_fashion_tags / mens_travel_tags / mens_gifts_tags:
    ...
    mens_gifts_tags: dict[str, list[str]] = {}
    ...
    if MENS_FASHION_ACC_RAW.exists():
        data = json.loads(MENS_FASHION_ACC_RAW.read_text())
        for row in data.get("products") or []:
            mens_fashion_stats["raw"] += 1
            row = dict(row)
            row["kind"] = "mens-fashion-accessories"
            sku = str(row.get("productCode") or row.get("id") or "")
            briq_id = f"gc-{sku.lower()}" if sku else ""
            mcols = mens_fashion_acc_cols_from_row(row)
            if sku.upper() in existing_skus:
                if mcols:
                    mens_fashion_tags[sku.upper()] = mcols
                    mens_fashion_stats["tagged_existing"] += 1
                mens_fashion_stats["skipped_existing_sku"] += 1
                continue
            if briq_id and briq_id in existing_ids:
                if mcols:
                    mens_fashion_tags[sku.upper()] = mcols
                    mens_fashion_stats["tagged_existing"] += 1
                mens_fashion_stats["skipped_existing_id"] += 1
                continue
            mens_fashion_stats["kept"] += 1
            remember(sku)
            rows.append(row)
    ...
    if MENS_TRAVEL_RAW.exists():
        data = json.loads(MENS_TRAVEL_RAW.read_text())
        for row in data.get("products") or []:
            mens_travel_stats["raw"] += 1
            row = dict(row)
            row["kind"] = "mens-travel"
            sku = str(row.get("productCode") or row.get("id") or "")
            briq_id = f"gc-{sku.lower()}" if sku else ""
            mcols = mens_travel_cols_from_row(row)
            if sku.upper() in existing_skus:
                if mcols:
                    mens_travel_tags[sku.upper()] = mcols
                    mens_travel_stats["tagged_existing"] += 1
                mens_travel_stats["skipped_existing_sku"] += 1
                continue
            if briq_id and briq_id in existing_ids:
                if mcols:
                    mens_travel_tags[sku.upper()] = mcols
                    mens_travel_stats["tagged_existing"] += 1
                mens_travel_stats["skipped_existing_id"] += 1
                continue
            mens_travel_stats["kept"] += 1
            remember(sku)
            rows.append(row)
    ...
    if MENS_GIFTS_RAW.exists():
        data = json.loads(MENS_GIFTS_RAW.read_text())
        for row in data.get("products") or []:
            mens_gifts_stats["raw"] += 1
            row = dict(row)
            row["kind"] = "mens-gifts"
            sku = str(row.get("productCode") or row.get("id") or "")
            mcols = mens_gifts_cols_from_row(row)
            if not mcols:
                continue
            if sku.upper() in existing_skus or row.get("tagOnly"):
                mens_gifts_tags[sku.upper()] = mcols
                mens_gifts_stats["tagged_existing"] += 1
                continue
            briq_id = f"gc-{sku.lower()}" if sku else ""
            if briq_id and briq_id in existing_ids:
                mens_gifts_tags[sku.upper()] = mcols
            ...
            sc = style_color_key(sku)
            if sc and sc in existing_style_colors:
                mens_gifts_stats["skipped_style_color"] += 1
                continue
            mens_gifts_stats["kept_new"] += 1
            remember(sku)
            rows.append(row)
...
def main() -> None:
    ...
    if not rows:
        raise SystemExit(
            ...
            "scrape-gc-jewellery-watches.py, scrape-gc-gifts.py "
            "and/or scrape-gc-mens-gifts.py first"
        )
    ...
    mens_fashion_tagged = apply_mens_fashion_acc_membership(
    ...
    mens_gifts_tagged = apply_mens_gifts_membership(products, mens_gifts_tags)
    ...
    OUT_TS.write_text(
        ...
        "men's fashion accessories + women's/men's travel + jewellery + gifts + "
        "men's gifts. */\n"
    ...
    mens_fashion_n = sum(
        1
        for p in products
        if "gc-men-fashion-accessories" in (p.get("gcCollections") or [])
        or any(
            c in MEN_FASHION_ACC_LEAF_COLLECTIONS
            for c in (p.get("gcCollections") or [])
        )
    )
    travel_n = sum(
    ...
    mens_travel_n = sum(
        1
        for p in products
        if "gc-men-travel" in (p.get("gcCollections") or [])
        or any(
            c in MEN_TRAVEL_LEAF_COLLECTIONS for c in (p.get("gcCollections") or [])
        )
    )
    jewellery_n = sum(
        1
    ...
    print(
        ...
        f"mens-fashion: {mens_fashion_n}  "
        f"travel: {travel_n}  mens-travel: {mens_travel_n}  jewellery: {jewellery_n}  gifts: {gifts_n}",
        flush=True,
    )
    ...
    if mens_fashion_stats["raw"]:
        skipped = (
            mens_fashion_stats["skipped_existing_sku"]
            + mens_fashion_stats["skipped_existing_id"]
        )
        print(
            f"  mens-fashion-acc raw={mens_fashion_stats['raw']} "
            f"kept={mens_fashion_stats['kept']} skipped={skipped} "
            f"tagged_existing={mens_fashion_stats.get('tagged_existing', 0)} "
            f"(merged onto products={mens_fashion_tagged}) "
            f"(sku={mens_fashion_stats['skipped_existing_sku']} "
            f"id={mens_fashion_stats['skipped_existing_id']})",
            flush=True,
        )
        for leaf in MEN_FASHION_ACC_LEAF_COLLECTIONS:
            n = sum(1 for p in products if leaf in (p.get("gcCollections") or []))
            print(f"    {leaf}: {n}", flush=True)
    if travel_stats["raw"]:
        print(
    ...
    if mens_travel_stats["raw"]:
        skipped = (
            mens_travel_stats["skipped_existing_sku"]
            + mens_travel_stats["skipped_existing_id"]
        )
        print(
            f"  mens-travel raw={mens_travel_stats['raw']} "
            f"kept={mens_travel_stats['kept']} skipped={skipped} "
            f"tagged_existing={mens_travel_stats.get('tagged_existing', 0)} "
            f"(merged onto products={mens_travel_tagged}) "
            f"(sku={mens_travel_stats['skipped_existing_sku']} "
            f"id={mens_travel_stats['skipped_existing_id']})",
            flush=True,
        )
    ...
    if mens_gifts_stats["raw"]:
        print(
            f"  mens-gifts raw={mens_gifts_stats['raw']} "
            f"new={mens_gifts_stats['kept_new']} "
            f"tagged_existing={mens_gifts_stats['tagged_existing']} "
            f"(merged onto products={mens_gifts_tagged}) "
            f"skipped_style_color={mens_gifts_stats['skipped_style_color']}",
            flush=True,
        )
        for leaf in MEN_GIFTS_LEAF_COLLECTIONS:
            n = sum(1 for p in products if leaf in (p.get("gcCollections") or []))
            print(f"    {leaf}: {n}", flush=True)
        n = sum(1 for p in products if "gc-men-gifts" in (p.get("gcCollections") or []))
        print(f"    gc-men-gifts: {n}", flush=True)
    ...
    if mens_travel_parent_only:
        print(f"  gc-men-travel (no leaf): {mens_travel_parent_only}", flush=True)
    jw_parent_only = sum(
        1
        for p in products
        if "gc-jewellery-watches" in (p.get("gcCollections") or [])
