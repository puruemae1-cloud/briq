#!/usr/bin/env python3
"""Enrich Dior women's accessories catalog: rich PDP copy, featuresKo, techSpecs."""
...
from di_common import WOMEN_ACCESSORIES_LEAVES, algolia_merch_hits_by_codes  # noqa: E402
...
RAW = ROOT / "src/data/di/di-women-accessories-catalog-raw.json"
CHECKPOINT_EVERY = 25

ACC_LEAVES = {
    "di-women-accessories",
    *[L["id"] for L in WOMEN_ACCESSORIES_LEAVES],
}
...
def story_sections_for_accessories(
    description_ko: str,
    images: list[str],
    *,
    ...
    detail_body = (
        ...
        if features_ko
        else "Dior 공식 제품 컷으로 확인하는 소재감과 시그니처 디테일입니다."
    )
    spec_parts: list[str] = []
    if material_ko:
    ...
    if len(images) > 6:
        sections.append(
            {
                "titleKo": "스타일링",
                "bodyKo": "데일리 룩부터 리조트 룩까지 포인트가 되는 디올 여성 액세서리입니다.",
...
def enrich_product(
    ...
    *,
    ...
    product["storySections"] = story_sections_for_accessories(
        description_ko,
        images,
...
def main() -> int:
    ...
    acc = [p for p in products if is_accessory(p)]
    if args.only:
        want = {x.strip() for x in args.only.split(",") if x.strip()}
        acc = [p for p in acc if p.get("sku") in want]

    if not acc:
        print("No accessories products in catalog — run scrape + merge first.", flush=True)
        return 0

    codes = [p["sku"] for p in acc if p.get("sku")]
    hits: dict[str, dict] = {}
    ...
    for i, p in enumerate(acc, 1):
        ...
        if i % CHECKPOINT_EVERY == 0:
            print(f"  checkpoint {i}/{len(acc)}", flush=True)
    ...
    save_translate_cache(cache)
    print(
        f"DONE accessories={len(acc)} features={stats['with_features']} "
