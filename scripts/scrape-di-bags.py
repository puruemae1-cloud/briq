#!/usr/bin/env python3
"""Scrape Dior GB Women's Bags by Category via Playwright WebKit.

Official leaves under Bags → Dior → 여성용 (~388 SKUs). Run in 3 stages
with a pause between for machine stability:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \\
    python3 scripts/scrape-di-bags.py --stage 1
  # pause ~5 min
  python3 scripts/scrape-di-bags.py --stage 2
  # pause ~5 min
  python3 scripts/scrape-di-bags.py --stage 3
...
OUT_RAW = ROOT / "src/data/di/di-bags-women-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-bags-women-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-bags-women-pdp-cache.json"

PDP_PAUSE = 0.8
HUB = f"{BASE}/{LANG}/fashion/womens-fashion/bags/all-the-bags"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_BAGS_WOMEN, leaf["id"]],
            ...
            for i, hit in enumerate(hits, start=1):
                ...
                if mh and not args.skip_pdp:
                    ...
                    if args.max_pdp and i > args.max_pdp:
                        ...
                        # PDP HTML often Akamai-blocked — keep PLP/merch only.
