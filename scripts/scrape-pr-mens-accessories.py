#!/usr/bin/env python3
"""Scrape Prada GB men's accessories (non-SLG) + PDP images.

Source hub: https://www.prada.com/gb/en/mens/accessories/c/10156EU
PLP membership uses Algolia CategoriesEnriched ``10156EU|false|false``.
Skips small leather goods (10346EU hub — scraped separately).
"""
...
OUT_RAW = ROOT / "src/data/pr/pr-mens-accessories-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-mens-accessories-pdp-cache.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"

BASE = "https://www.prada.com"
HUB_URL = f"{BASE}/gb/en/mens/accessories/c/10156EU"
HUB_CID = "10156EU"
...
# Official leaf PLPs under men's accessories (prada.com GB hub menu)
LEAVES: list[tuple[str, str, str]] = [
    ("pr-men-sunglasses", "Sunglasses", "10163EU"),
    ("pr-men-hats-gloves", "Hats and gloves", "10160EU"),
    ("pr-men-bag-charms", "Bag charms and keychains", "10161EU"),
    ("pr-men-belts", "Belts", "10157EU"),
    ("pr-men-custom-belts", "Custom belts", "10158EU"),
    ("pr-men-silks-scarves", "Silks and scarves", "10162EU"),
    ("pr-men-ties-bow-ties", "Ties and bow ties", "10164EU"),
    ("pr-men-jewels", "Jewels", "10622EU"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid in LEAVES}
# Algolia-only jewellery subcategories map to the Jewels leaf PLP
LEAF_BY_CID.update(
    {
        "10170EU": "pr-men-jewels",
        "10172EU": "pr-men-jewels",
        "10174EU": "pr-men-jewels",
    }
)
BC_TO_LEAF = {
    "Sunglasses": "pr-men-sunglasses",
    "Hats and gloves": "pr-men-hats-gloves",
    "Bag charms and keychains": "pr-men-bag-charms",
    "Belts": "pr-men-belts",
    "Custom belts": "pr-men-custom-belts",
    "Silks and scarves": "pr-men-silks-scarves",
    "Ties and bow ties": "pr-men-ties-bow-ties",
    "Fashion Jewelry": "pr-men-jewels",
    "Silver Jewelry": "pr-men-jewels",
    "Jewels": "pr-men-jewels",
}
LEAF_SLUG = {
    "pr-men-sunglasses": "sunglasses",
    "pr-men-hats-gloves": "hats-and-gloves",
    "pr-men-bag-charms": "bag-charms-and-keychains",
    "pr-men-belts": "belts",
    "pr-men-custom-belts": "custom-belts",
    "pr-men-silks-scarves": "silks-and-scarves",
    "pr-men-ties-bow-ties": "ties-and-bow-ties",
    "pr-men-jewels": "jewels",
}
...
SLG_CIDS = {"10346EU", "10347EU", "10348EU", "10349EU", "10350EU"}
SKIP_BC = {"Travel accessories", "Fragrances"}

PARENT_COLS = ["prada", "prada-accessories", "pr-mens-accessories"]
MAX_WORKERS = 6
IMG_WORKERS = 10
MAX_IMAGES = 10
...
SLG_ENRICHED_PREFIXES = tuple(f"{cid}|" for cid in sorted(SLG_CIDS | {"10346EU"}))


def is_slg_hit(hit: dict) -> bool:
    bc = ((hit.get("Breadcrumbs") or {}).get("level_3") or {}).get("en_GB") or ""
    if bc in SLG_BC or bc in SKIP_BC:
        return True
    cats = set(hit.get("Categories") or [])
    if cats & SLG_CIDS:
        return True
    for enriched in hit.get("CategoriesEnriched") or []:
        if any(str(enriched).startswith(prefix) for prefix in SLG_ENRICHED_PREFIXES):
            return True
    return False
...
def infer_leaf_from_name(name: str) -> str | None:
    ...
    if "sunglass" in n or "eyewear" in n:
        return "pr-men-sunglasses"
    if "bow tie" in n or "bowtie" in n:
        return "pr-men-ties-bow-ties"
    if " tie" in n or n.endswith(" tie") or n.startswith("tie "):
        return "pr-men-ties-bow-ties"
    if any(x in n for x in ("scarf", "bandana", "stole")):
        return "pr-men-silks-scarves"
    if "silk" in n and "tie" not in n:
        return "pr-men-silks-scarves"
    if any(x in n for x in ("glove", "beanie", "hat", "cap")):
        return "pr-men-hats-gloves"
    if any(x in n for x in ("keychain", "key ring", "charm", "key holder")):
        return "pr-men-bag-charms"
    if any(x in n for x in ("ring", "necklace", "bracelet", "earring", "brooch", "jewel")):
        return "pr-men-jewels"
    if "belt" in n:
        if "custom" in n:
            return "pr-men-custom-belts"
        return "pr-men-belts"
    return None
...
def primary_leaf(cols: list[str], hit: dict) -> str:
    ...
    for lid, _label, _cid in LEAVES:
        if lid in cols:
            return lid
    name = ((hit.get("ProductName") or {}).get("en_GB") or "")
    return infer_leaf_from_name(name) or "pr-men-sunglasses"
...
def main() -> None:
    ...
    print("=== Prada men's accessories hub (Algolia)", flush=True)
    ...
    for lid, label, cid in LEAVES:
        ...
        plp_meta[lid] = {
            "label": label,
            "categoryCode": cid,
            "url": f"{BASE}/gb/en/mens/accessories/{LEAF_SLUG[lid]}/c/{cid}",
            "count": n,
        }
