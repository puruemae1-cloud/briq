#!/usr/bin/env python3
"""Scrape Dior GB Men's Bags by Category via Playwright WebKit.

Official leaves under Bags → Dior → 남성용 (~163 SKUs). Run in 3 stages
with a pause between for machine stability:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \\
    python3 scripts/scrape-di-bags-men.py --stage 1
  # pause ~5 min
  python3 scripts/scrape-di-bags-men.py --stage 2
  # pause ~5 min
  python3 scripts/scrape-di-bags-men.py --stage 3
...
OUT_RAW = ROOT / "src/data/di/di-bags-men-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-bags-men-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-bags-men-pdp-cache.json"

PDP_PAUSE = 0.8
HUB = f"{BASE}/{LANG}/fashion/mens-fashion/bags/all-bags"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_BAGS_MEN, leaf["id"]],
