...
def scrape() -> dict:
    ...
    # Attach cached official accordion details (Details / Fit / Care) when missing.
    if DETAILS_HANDLE.exists():
        try:
            det_cache = json.loads(DETAILS_HANDLE.read_text())
        except Exception:
            det_cache = {}
        if isinstance(det_cache, dict):
            filled = 0
            for p in products:
                det = det_cache.get(p.get("handle") or "")
                ...
                for key in ("details", "fit", "care"):
                    ...
                    if isinstance(val, list) and val:
                        ...
                        filled += 1
            if filled:
                print(f"Applied details-cache fields={filled}", flush=True)
