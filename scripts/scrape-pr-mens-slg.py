#!/usr/bin/env python3
"""Scrape Prada GB men's small leather goods (~140) in stages.

Hub: https://www.prada.com/gb/en/mens/small-leather-goods/c/10346EU
PLP set = Algolia CategoriesEnriched ``10346EU|false|false``.

  python3 scripts/scrape-pr-mens-slg.py --stage 1 --stages 2
  python3 scripts/scrape-pr-mens-slg.py --refresh-sizes
"""
...
OUT_RAW = ROOT / "src/data/pr/pr-mens-slg-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-mens-slg-pdp-cache.json"
SEED = ROOT / "src/data/pr/pr-mens-slg-hub-seed.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"
PROGRESS = ROOT / "src/data/pr/pr-mens-slg-stage-progress.json"

BASE = "https://www.prada.com"
HUB_URL = f"{BASE}/gb/en/mens/small-leather-goods/c/10346EU"
HUB_CID = "10346EU"
...
# Official leaf PLPs under men's small leather goods (prada.com GB hub)
LEAVES: list[tuple[str, str, str]] = [
    ("pr-men-card-holders", "Card holders", "10349EU"),
    ("pr-men-small-wallets", "Small wallets", "10347EU"),
    ("pr-men-large-wallets", "Large wallets", "10348EU"),
    ("pr-men-high-tech-accessories", "High-Tech accessories", "10350EU"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid in LEAVES}
BC_TO_LEAF = {
    "Card holders": "pr-men-card-holders",
    "Small wallets": "pr-men-small-wallets",
    "Large wallets": "pr-men-large-wallets",
    "High-Tech accessories": "pr-men-high-tech-accessories",
    "High-tech accessories": "pr-men-high-tech-accessories",
}
LEAF_SLUG = {
    "pr-men-card-holders": "card-holders",
    "pr-men-small-wallets": "small-wallets",
    "pr-men-large-wallets": "large-wallets",
    "pr-men-high-tech-accessories": "high-tech-accessories",
}

PARENT_COLS = ["prada", "prada-accessories", "pr-mens-accessories", "pr-mens-slg"]
MAX_WORKERS = 4
IMG_WORKERS = 4
MAX_IMAGES = 10
...
def collections_for(hit: dict) -> list[str]:
    ...
    for cid in hit.get("Categories") or []:
        ...
        if lid:
            cols.add(lid)
    if not any(c.startswith("pr-men-") and c != "pr-mens-slg" for c in cols):
...
def main() -> None:
    ...
    plp_meta = {
        "pr-mens-slg": {
            "label": "Men's small leather goods",
    ...
    for lid, label, cid in LEAVES:
        ...
        plp_meta[lid] = {
            ...
            "categoryCode": cid,
            "count": n,
            "url": f"{BASE}/gb/en/mens/small-leather-goods/{slug}/c/{cid}",
        }
