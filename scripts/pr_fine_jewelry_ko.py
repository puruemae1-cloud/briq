#!/usr/bin/env python3
"""Curated Korean copy for Prada Fine Jewelry."""
from __future__ import annotations

import json
from pathlib import Path

_DATA = Path(__file__).resolve().parents[1] / "src/data/pr/pr-fine-jewelry-ko-copy.json"
...
def seed_fine_jewelry_cache(cache: dict[str, str]) -> int:
    n = 0
...
def fine_jewelry_text_ko(text: str | None) -> str | None:
    s = (text or "").strip()
    if not s:
        return ""
    for mapping in (TITLE_KO, COLOR_KO, DESCRIPTION_KO, DETAIL_KO, MATERIAL_KO):
