#!/usr/bin/env python3
"""Scrape Dior GB Maison → Tableware via Playwright WebKit.

Stages (≈⅓ of ~406 unique items each):
  1 — Plates & Bowls
  2 — Glasses + Carafes + Cutlery
  3 — Tea & Coffee (+ fill All Tableware gaps)

  python3 -m playwright install webkit
  python3 scripts/scrape-di-tableware.py --stage 1
...
OUT_RAW = ROOT / "src/data/di/di-tableware-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-tableware-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-tableware-pdp-cache.json"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS, leaf["id"]],
...
def main() -> None:
    ...
    leaves = [
        L
        for L in TABLEWARE_LEAVES
        if args.stage == "all" or L.get("stage") == str(args.stage)
    ]
    # stage 1 should not include "all" leaf
    if args.stage == "1":
        leaves = [L for L in leaves if L["id"] == "di-plates-bowls"]
    elif args.stage == "2":
        leaves = [L for L in leaves if L["id"] in ("di-glasses", "di-carafes", "di-cutlery")]
    elif args.stage == "3":
        leaves = [L for L in leaves if L["id"] in ("di-tea-coffee", "di-tableware-all")]

    save_json(OUT_LEAVES, TABLEWARE_LEAVES)
    ...
    with sync_playwright() as p:
        ...
        page.goto(
            f"{BASE}/{LANG}/fashion/maison/tableware/all-tableware",
        ...
        for leaf in leaves:
            ...
            save_json(
                OUT_RAW,
                {
                    ...
                    "stage": args.stage,
                    "hub": f"{BASE}/{LANG}/fashion/maison/tableware/all-tableware",
                    "leaves": TABLEWARE_LEAVES,
