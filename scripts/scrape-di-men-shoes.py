#!/usr/bin/env python3
"""Scrape Dior GB Men's Shoes via Playwright WebKit.

Official leaves under Shoes → Dior → 남성용 (~207 SKUs from all-shoes). Run in 3
stages with a pause between for machine stability:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \\
    python3 scripts/scrape-di-men-shoes.py --stage 1
  # pause ~5 min
  python3 scripts/scrape-di-men-shoes.py --stage 2
  # pause ~5 min
  python3 scripts/scrape-di-men-shoes.py --stage 3
...
OUT_RAW = ROOT / "src/data/di/di-men-shoes-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-men-shoes-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-men-shoes-pdp-cache.json"

HUB = f"{BASE}/{LANG}/fashion/mens-fashion/shoes/all-shoes"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_MEN_SHOES, leaf["id"]],
        ...
        for leaf in leaves:
            ...
            for i, hit in enumerate(hits, start=1):
                ...
                if prev:
                    ...
                    if leaf["id"] in ("di-men-shoes-all",) and prev.get("leafId"):
                        pl = prev["leafId"]
                        if pl not in ("di-men-shoes-all",):
                            row["leafId"] = pl
