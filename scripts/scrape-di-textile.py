#!/usr/bin/env python3
"""Scrape Dior GB Maison → Textile via Playwright WebKit.

Single run (~79 SKUs across official textile leaves).

  python3 -m playwright install webkit
  python3 scripts/scrape-di-textile.py --stage all
...
OUT_RAW = ROOT / "src/data/di/di-textile-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-textile-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-textile-pdp-cache.json"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_TEXTILE, leaf["id"]],
...
def main() -> None:
    ...
    ap.add_argument("--stage", type=str, default="all", help="all (default) | leaf id")
    ...
    if args.stage == "all":
        leaves = list(TEXTILE_LEAVES)
    else:
        leaves = [L for L in TEXTILE_LEAVES if L["id"] == args.stage or L.get("stage") == str(args.stage)]
        if not leaves:
            leaves = list(TEXTILE_LEAVES)

    save_json(OUT_LEAVES, TEXTILE_LEAVES)
    ...
    with sync_playwright() as p:
        ...
        page = ctx.new_page()
        page.goto(
            f"{BASE}/{LANG}/fashion/maison/textile/all-textiles",
        ...
        for leaf in leaves:
            ...
            save_json(
                OUT_RAW,
                {
                    ...
                    "stage": args.stage,
                    "hub": f"{BASE}/{LANG}/fashion/maison/textile/all-textiles",
                    "leaves": TEXTILE_LEAVES,
