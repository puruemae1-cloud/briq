#!/usr/bin/env python3
"""Scrape Dior GB Women's Ready-to-Wear by Category via Playwright WebKit.

Official leaves under Luxury → Dior → 여성용 (~549 SKUs). Run in 4 stages
with a pause between for machine stability:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \\
    python3 scripts/scrape-di-women-rtw.py --stage 1
  # pause ~5 min
  python3 scripts/scrape-di-women-rtw.py --stage 2
  # pause ~5 min
  python3 scripts/scrape-di-women-rtw.py --stage 3
  # pause ~5 min
  python3 scripts/scrape-di-women-rtw.py --stage 4
...
OUT_RAW = ROOT / "src/data/di/di-women-rtw-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-women-rtw-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-women-rtw-pdp-cache.json"

PDP_PAUSE = 0.8
HUB = f"{BASE}/{LANG}/fashion/womens-fashion/ready-to-wear/all-ready-to-wear"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_WOMEN_RTW, leaf["id"]],
...
def main() -> None:
    ...
    ap.add_argument("--stage", type=str, default="all", help="all | 1 | 2 | 3 | 4 | leaf id")
