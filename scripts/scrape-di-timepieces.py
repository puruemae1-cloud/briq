#!/usr/bin/env python3
"""Scrape Dior GB Jewelry & Timepieces → Timepieces by Collection via Playwright WebKit.

Single run (~26 SKUs across official timepieces-by-collection leaves).

  python3 -m playwright install webkit
  python3 scripts/scrape-di-timepieces.py --stage all
...
OUT_RAW = ROOT / "src/data/di/di-timepieces-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-timepieces-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-timepieces-pdp-cache.json"

PDP_PAUSE = 1.0
HUB = f"{BASE}/{LANG}/fashion/jewelry-timepieces/timepieces-by-collection/all-pieces"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_TIMEPIECE, leaf["id"]],
...
def main() -> None:
    ...
    if args.stage == "all":
        leaves = list(TIMEPIECE_LEAVES)
    else:
        leaves = [
            L
            for L in TIMEPIECE_LEAVES
            if L["id"] == args.stage or L.get("stage") == str(args.stage)
        ]
        if not leaves:
            leaves = list(TIMEPIECE_LEAVES)

    save_json(OUT_LEAVES, TIMEPIECE_LEAVES)
