#!/usr/bin/env python3
"""Scrape Dior GB Men's Ready-to-Wear by Category via Playwright WebKit.

Official leaves under Luxury → Dior → 남성용 (~549 SKUs). Run in 4 stages
with a pause between for machine stability:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \\
    python3 scripts/scrape-di-men-rtw.py --stage 1
  # pause ~5 min
  python3 scripts/scrape-di-men-rtw.py --stage 2
  # pause ~5 min
  python3 scripts/scrape-di-men-rtw.py --stage 3
  # pause ~5 min
  python3 scripts/scrape-di-men-rtw.py --stage 4
...
OUT_RAW = ROOT / "src/data/di/di-men-rtw-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-men-rtw-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-men-rtw-pdp-cache.json"

PDP_PAUSE = 0.8
HUB = f"{BASE}/{LANG}/fashion/mens-fashion/ready-to-wear/all-ready-to-wear"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_MEN_RTW, leaf["id"]],
