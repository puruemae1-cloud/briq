#!/usr/bin/env python3
"""Scrape Dior GB Women's Accessories via Playwright WebKit.

Official leaves under Accessories → Dior → 여성용. Run in 3 stages:

  PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright \
    python3 scripts/scrape-di-women-accessories.py --stage 1
  python3 scripts/scrape-di-women-accessories.py --stage 2
  python3 scripts/scrape-di-women-accessories.py --stage 3
  python3 scripts/merge-di-catalog-ko.py
  python3 scripts/enrich-di-women-accessories-pdp.py
...
OUT_RAW = ROOT / "src/data/di/di-women-accessories-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-women-accessories-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-women-accessories-pdp-cache.json"

HUB = f"{BASE}/{LANG}/fashion/womens-fashion/accessories/all-accessories"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_WOMEN_ACCESSORIES, leaf["id"]],
...
def main() -> None:
    ...
    if args.stage == "all":
        leaves = list(WOMEN_ACCESSORIES_LEAVES)
    else:
        leaves = [
            L
            for L in WOMEN_ACCESSORIES_LEAVES
            if L["id"] == args.stage or L.get("stage") == str(args.stage)
        ]
        if not leaves:
            raise SystemExit(f"unknown stage/leaf: {args.stage}")

    save_json(OUT_LEAVES, WOMEN_ACCESSORIES_LEAVES)
        ...
        for leaf in leaves:
            ...
            for i, hit in enumerate(hits, start=1):
                ...
                if prev:
                    ...
                    if leaf["id"] in ("di-women-acc-all",) and prev.get("leafId"):
                        pl = prev["leafId"]
                        if pl not in ("di-women-acc-all",):
                            row["leafId"] = pl
                ...
                if i % 15 == 0:
                    ...
                    save_json(
                        OUT_RAW,
                        {
                            ...
                            "stage": args.stage,
                            "hub": HUB,
                            "leaves": WOMEN_ACCESSORIES_LEAVES,
