#!/usr/bin/env python3
"""Scrape Burberry Beauty hub PLPs and merge bb-beauty-* into bb-catalog-raw.json."""
...
def main() -> None:
    ...
    for coll_id, label, path, top, _parent in BEAUTY_COLLECTIONS:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            ...
            for fut in as_completed(futs):
                ...
                if done % 25 == 0 or done == len(need):
                    ...
                    print(f"  Beauty PDP {done}/{len(need)}", flush=True)
                if err:
    ...
    PDP_CACHE.write_text(json.dumps(cache, ensure_ascii=False))
    print(
        f"Wrote {RAW_PATH} products={len(products)} beauty_errors={len(errors)}",
        flush=True,
    )
