#!/usr/bin/env python3
"""Scrape Dior GB Maison → Decor via Playwright WebKit.
...
Stages (pause ~5 min between for machine stability):
  ...
  python3 scripts/scrape-di-decor.py --stage 1
...
OUT_RAW = ROOT / "src/data/di/di-decor-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-decor-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-decor-pdp-cache.json"
...
def parse_pdp(page, path: str) -> dict:
    url = path if path.startswith("http") else f"{BASE}{path}"
    page.goto(url, wait_until="domcontentloaded", timeout=120000)
    time.sleep(1.5)
    data = extract_next_data(page.content()) or {}
    pp = (data.get("props") or {}).get("pageProps") or {}
    product = pp.get("product") or {}
    title = (product.get("title") or product.get("name") or "").strip()
    desc = clean_dior_description(product.get("description") or "")
    gallery: list[str] = []
    for media in product.get("medias") or product.get("images") or []:
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_DECOR, leaf["id"]],
...
def main() -> None:
    ...
    ap.add_argument("--stage", type=str, default="1", help="1 | 2 | 3 | all")
    ...
    if args.stage == "1":
        leaves = [
            L
            for L in DECOR_LEAVES
            if L["id"] in ("di-decorative-pieces", "di-vases")
        ]
    elif args.stage == "2":
        leaves = [
            L
            for L in DECOR_LEAVES
            if L["id"] in ("di-lighting", "di-baskets", "di-wallpapers")
        ]
    elif args.stage == "3":
        leaves = [
            L for L in DECOR_LEAVES if L["id"] in ("di-furniture", "di-decor-all")
        ]
    elif args.stage == "all":
        leaves = list(DECOR_LEAVES)
    else:
        leaves = [L for L in DECOR_LEAVES if L.get("stage") == str(args.stage)]

    save_json(OUT_LEAVES, DECOR_LEAVES)
    ...
    with sync_playwright() as p:
        ...
        page = ctx.new_page()
        page.goto(
            f"{BASE}/{LANG}/fashion/maison/decor/all-products",
        ...
        for leaf in leaves:
                ...
                if mh and not args.skip_pdp:
                    ...
                    if args.max_pdp and i > args.max_pdp:
                        ...
                        try:
                            ...
                            pdp = parse_pdp(
                                page, link or f"/{LANG}/fashion/products/{code}"
                            )
                            pdp_cache[code] = pdp
                            if i % 10 == 0:
            ...
            save_json(
                OUT_RAW,
                {
                    ...
                    "stage": args.stage,
                    "hub": f"{BASE}/{LANG}/fashion/maison/decor/all-products",
                    "leaves": DECOR_LEAVES,
