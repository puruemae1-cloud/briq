#!/usr/bin/env python3
"""Scrape Gucci UK Gifts PLPs + PDP images for *new* SKUs only.

Existing catalogue productCodes are recorded with gift membership only
(no second PDP / no re-download). Official hub:
https://www.gucci.com/uk/en_gb/nst/gifts

Leaves (categoryCode → Briq id):
  gifts-for-her              → gc-gifts-her
  gifts-for-him              → gc-gifts-him
  monogramming-gifts-for-her → gc-gifts-personalised
  monogramming-gifts-for-him → gc-gifts-personalised
  beauty-gift-ideas          → gc-gifts-beauty
  jewelry-watches-fashion-jewelry → gc-gifts-jewellery
  gifts-gifts-for-children   → gc-gifts-children
"""
...
OUT_RAW = ROOT / "src/data/gc/gc-gifts-catalog-raw.json"
...
WARM_URL = f"{BASE}/uk/en_gb/nst/gifts"

# (briq_id, label, categoryCode)
COLLECTIONS: list[tuple[str, str, str]] = [
    ("gc-gifts-her", "Gifts for Her", "gifts-for-her"),
    ("gc-gifts-him", "Gifts for Him", "gifts-for-him"),
    (
        "gc-gifts-personalised",
        "Personalised Gifts / For Her",
        "monogramming-gifts-for-her",
    ),
    (
        "gc-gifts-personalised",
    ...
    ("gc-gifts-beauty", "Fragrances and Make-up Gifts", "beauty-gift-ideas"),
    (
        "gc-gifts-jewellery",
        "Jewellery Gifts",
        "jewelry-watches-fashion-jewelry",
    ),
    ("gc-gifts-children", "Gifts for Children", "gifts-gifts-for-children"),
]
...
PARENT_COLLS = ("gc-gifts", "gucci-accessories")
...
def main() -> None:
    ...
    for coll_id, label, cat_code in COLLECTIONS:
        ...
        try:
            ...
            items = []
        prev_meta = plp_meta.get(coll_id) or {
            "label": (
                "Personalised Gifts"
                if coll_id == "gc-gifts-personalised"
                else label.split(" / ")[0]
            ),
            "categoryCodes": [],
            "count": 0,
        }
        prev_meta["categoryCodes"] = sorted(
            set(prev_meta.get("categoryCodes") or []) | {cat_code}
        )
        plp_meta[coll_id] = prev_meta
    ...
    print(
        f"Unique gift colourways: {len(codes)} "
    ...
    if new_codes:
        print(f"Enriching {len(new_codes)} new gift SKUs…", flush=True)
    ...
    payload = {
        ...
        "note": (
            "Official gifts hub leaves. Personalised = monogramming her+him. "
            "Jewellery Gifts → fashion jewellery PLP. Existing SKUs are tagOnly."
        ),
        "products": products,
    }
