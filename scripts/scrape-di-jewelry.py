#!/usr/bin/env python3
"""Scrape Dior GB Jewelry & Timepieces → Jewelry by Category via Playwright WebKit.

Single run (~167 SKUs across official jewelry-by-category leaves).

  python3 -m playwright install webkit
  python3 scripts/scrape-di-jewelry.py --stage all
...
OUT_RAW = ROOT / "src/data/di/di-jewelry-catalog-raw.json"
OUT_LEAVES = ROOT / "src/data/di/di-jewelry-leaves.json"
PDP_CACHE = ROOT / "src/data/di/di-jewelry-pdp-cache.json"

PDP_PAUSE = 1.0
HUB = f"{BASE}/{LANG}/fashion/jewelry-timepieces/jewelry-by-category/all-jewelry"
...
def hit_to_row(hit: dict, leaf: dict, pdp: dict | None) -> dict:
    ...
    return {
        ...
        "collections": [*PARENT_COLS_JEWELRY, leaf["id"]],
...
def main() -> None:
    ...
    if args.stage == "all":
        leaves = list(JEWELRY_LEAVES)
    else:
        leaves = [
            L
            for L in JEWELRY_LEAVES
            if L["id"] == args.stage or L.get("stage") == str(args.stage)
        ]
        if not leaves:
            leaves = list(JEWELRY_LEAVES)

    save_json(OUT_LEAVES, JEWELRY_LEAVES)
