#!/usr/bin/env python3
"""Scrape Prada GB men's handbags (hub + official leaf categories) + PDP images.

Source hub: https://www.prada.com/gb/en/mens/bags/c/10143EU
PLP membership uses Algolia CategoriesEnriched ``10143EU|false|false`` (~118 SKUs,
...
OUT_RAW = ROOT / "src/data/pr/pr-mens-handbags-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-mens-handbags-pdp-cache.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"

BASE = "https://www.prada.com"
HUB_URL = f"{BASE}/gb/en/mens/bags/c/10143EU"

ALGOLIA_APP = "OCPT799JD8"
ALGOLIA_KEY = "ff0caf66bf2f4d3b10b59c95711ddaf8"
ALGOLIA_INDEX = "PLP_COLOR_PRADA_Online_GB"
HUB_ENRICHED = "10143EU|false|false"

# Official leaf PLPs under men's bags
LEAVES: list[tuple[str, str, str, str]] = [
    # (briq id, EN label, category code, URL slug)
    ("pr-men-backpacks-belt-bags", "Backpacks and belt bags", "10144EU", "backpacks-and-belt-bags"),
    ("pr-men-briefcases", "Briefcases", "10145EU", "briefcases"),
    ("pr-men-clutches", "Clutches", "10146EU", "clutches"),
    ("pr-men-messenger-bags", "Messenger bags", "10147EU", "messenger-bags"),
    ("pr-men-tote-bags", "Totes", "10148EU", "totes"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid, _slug in LEAVES}
BC_TO_LEAF = {
    "Backpacks and belt bags": "pr-men-backpacks-belt-bags",
    "Backpacks": "pr-men-backpacks-belt-bags",
    "Belt bags": "pr-men-backpacks-belt-bags",
    "Briefcases": "pr-men-briefcases",
    "Clutches": "pr-men-clutches",
    "Messenger bags": "pr-men-messenger-bags",
    "Totes": "pr-men-tote-bags",
    "Pouches": "pr-men-clutches",
    "Duffle bags": "pr-men-tote-bags",
    "Duffel bags": "pr-men-tote-bags",
}

NAME_TO_LEAF = [
    ("duffel", "pr-men-tote-bags"),
    ("duffle", "pr-men-tote-bags"),
    ("backpack", "pr-men-backpacks-belt-bags"),
    ("belt bag", "pr-men-backpacks-belt-bags"),
    ("briefcase", "pr-men-briefcases"),
    ("clutch", "pr-men-clutches"),
    ("messenger", "pr-men-messenger-bags"),
    ("tote", "pr-men-tote-bags"),
    ("shoulder", "pr-men-messenger-bags"),
    ("explore", "pr-men-messenger-bags"),
]

PARENT_COLS = ["prada", "prada-bags", "pr-mens-handbags"]
MAX_WORKERS = 6
IMG_WORKERS = 8
MAX_IMAGES = 12
...
def collections_for(hit: dict) -> list[str]:
    ...
    for cid in cats:
        ...
        if lid:
            cols.add(lid)
    if not any(c.startswith("pr-men-") for c in cols):
        bc = ((hit.get("Breadcrumbs") or {}).get("level_3") or {}).get("en_GB") or ""
        fallback = BC_TO_LEAF.get(bc)
        if fallback:
            cols.add(fallback)
    if not any(c.startswith("pr-men-") for c in cols):
        name = ((hit.get("ProductName") or {}).get("en_GB") or "").lower()
        for needle, lid in NAME_TO_LEAF:
            if needle in name:
                cols.add(lid)
                break
    return sorted(cols)
...
def main() -> None:
    ...
    print("=== Prada men's bags hub (Algolia)", flush=True)
    ...
    for lid, label, cid, slug in LEAVES:
        ...
        plp_meta[lid] = {
            ...
            "categoryCode": cid,
            "count": n,
            "url": f"{BASE}/gb/en/mens/bags/{slug}/c/{cid}",
        }
