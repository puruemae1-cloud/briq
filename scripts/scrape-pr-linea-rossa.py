#!/usr/bin/env python3
"""Scrape Prada GB Linea Rossa + PDP images.

Landing: https://www.prada.com/gb/en/prada_linea_rossa.html
Official menu/leaf PLPs:
  - Women's Collection  10118EU
  - Men's Collection    10187EU
  - Sunglasses          10192EU
  - Shoes               10190EU
  - Fragrances          10292EU

Product set = union of Algolia CategoriesEnriched for women's + men's hubs.
"""
...
OUT_RAW = ROOT / "src/data/pr/pr-linea-rossa-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-linea-rossa-pdp-cache.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"

BASE = "https://www.prada.com"
LANDING_URL = f"{BASE}/gb/en/prada_linea_rossa.html"
WOMEN_HUB_CID = "10118EU"
MEN_HUB_CID = "10187EU"
HUB_URL = f"{BASE}/gb/en/mens/prada-linea-rossa/c/{MEN_HUB_CID}"

ALGOLIA_APP = "OCPT799JD8"
ALGOLIA_KEY = "ff0caf66bf2f4d3b10b59c95711ddaf8"
ALGOLIA_INDEX = "PLP_COLOR_PRADA_Online_GB"

# Official Linea Rossa link categories (landing + hub menu)
LEAVES: list[tuple[str, str, str]] = [
    ("pr-linea-rossa-women", "Women's Collection", WOMEN_HUB_CID),
    ("pr-linea-rossa-men", "Men's Collection", MEN_HUB_CID),
    ("pr-linea-rossa-sunglasses", "Sunglasses", "10192EU"),
    ("pr-linea-rossa-shoes", "Shoes", "10190EU"),
    ("pr-linea-rossa-fragrances", "Fragrances", "10292EU"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid in LEAVES}
# America's Cup sneakers live under shoes leaf on Linea Rossa
LEAF_BY_CID["10297EU"] = "pr-linea-rossa-shoes"

BC_TO_LEAF = {
    "Sunglasses": "pr-linea-rossa-sunglasses",
    "Fragrances": "pr-linea-rossa-fragrances",
    "Shoes": "pr-linea-rossa-shoes",
    "America's Cup": "pr-linea-rossa-shoes",
    "Ankle boots and boots": "pr-linea-rossa-shoes",
    "Hats and gloves": "pr-linea-rossa-men",
    "Swimwear": "pr-linea-rossa-women",
    "Sports and leisure": "pr-linea-rossa-women",
}
LEAF_SLUG = {
    "pr-linea-rossa-women": "",
    "pr-linea-rossa-men": "",
    "pr-linea-rossa-sunglasses": "sunglasses",
    "pr-linea-rossa-shoes": "shoes",
    "pr-linea-rossa-fragrances": "fragrances",
}
LEAF_URL = {
    "pr-linea-rossa-women": f"{BASE}/gb/en/womens/prada-linea-rossa/c/{WOMEN_HUB_CID}",
    "pr-linea-rossa-men": f"{BASE}/gb/en/mens/prada-linea-rossa/c/{MEN_HUB_CID}",
    "pr-linea-rossa-sunglasses": f"{BASE}/gb/en/mens/prada-linea-rossa/sunglasses/c/10192EU",
    "pr-linea-rossa-shoes": f"{BASE}/gb/en/mens/prada-linea-rossa/shoes/c/10190EU",
    "pr-linea-rossa-fragrances": f"{BASE}/gb/en/mens/prada-linea-rossa/fragrances/c/10292EU",
}

PARENT_COLS = ["prada", "prada-accessories", "pr-linea-rossa"]
MAX_WORKERS = 6
IMG_WORKERS = 10
MAX_IMAGES = 10
...
def fetch_hub_hits(s: cffi_requests.Session) -> list[dict]:
    """Union women's + men's Linea Rossa hubs (dedupe by objectID)."""
    by_id: dict[str, dict] = {}
    for cid in (WOMEN_HUB_CID, MEN_HUB_CID):
...
def infer_leaf_from_name(name: str) -> str | None:
    ...
    if "sunglass" in n or "eyewear" in n:
        return "pr-linea-rossa-sunglasses"
    if any(x in n for x in ("fragrance", "edp", "edt", "perfume", "luna rossa")):
        return "pr-linea-rossa-fragrances"
    if any(
        x in n
        for x in (
            "sneaker",
...
def collections_for(hit: dict) -> list[str]:
    ...
    for cid in cats:
        ...
        if lid:
            cols.add(lid)
    # gender hubs
    if WOMEN_HUB_CID in cats:
        cols.add("pr-linea-rossa-women")
    if MEN_HUB_CID in cats:
        cols.add("pr-linea-rossa-men")
    if not any(
        c
        in {
        ...
        if fallback:
            cols.add(fallback)
    if not any(c.startswith("pr-linea-rossa-") for c in cols):
...
def primary_leaf(cols: list[str], hit: dict) -> str:
    # Prefer specific product-type leaves over gender hubs
    for preferred in (
        "pr-linea-rossa-sunglasses",
        "pr-linea-rossa-shoes",
        "pr-linea-rossa-fragrances",
    ...
    if mapped and mapped in cols:
        return mapped
    for lid in ("pr-linea-rossa-women", "pr-linea-rossa-men"):
        if lid in cols:
            return lid
    name = ((hit.get("ProductName") or {}).get("en_GB") or "")
    return infer_leaf_from_name(name) or "pr-linea-rossa-men"
...
def main() -> None:
    ...
    print("=== Prada Linea Rossa (women + men hubs)", flush=True)
    ...
    plp_meta = {
        "pr-linea-rossa": {
            ...
            "categoryCode": f"{WOMEN_HUB_CID}+{MEN_HUB_CID}",
            "count": len(products),
            "url": LANDING_URL,
        }
    }
