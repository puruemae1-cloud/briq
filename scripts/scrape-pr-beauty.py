#!/usr/bin/env python3
"""Scrape Prada GB Beauty + PDP images.

Hub: https://www.prada.com/gb/en/perfumes-and-beauty/beauty/c/10565EU
Official leaf PLPs under Beauty:
  - Face                     10568EU
  - Eyes                     10570EU
  - Lips                     11102EU
  - Skincare                 10569EU
  - Brushes and accessories  10571EU
...
OUT_RAW = ROOT / "src/data/pr/pr-beauty-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-beauty-pdp-cache.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"

BASE = "https://www.prada.com"
HUB_CID = "10565EU"
LANDING_URL = f"{BASE}/gb/en/perfumes-and-beauty/beauty/c/{HUB_CID}"
HUB_URL = LANDING_URL
...
LEAVES: list[tuple[str, str, str]] = [
    ("pr-beauty-face", "Face", "10568EU"),
    ("pr-beauty-eyes", "Eyes", "10570EU"),
    ("pr-beauty-lips", "Lips", "11102EU"),
    ("pr-beauty-skincare", "Skincare", "10569EU"),
    ("pr-beauty-brushes", "Brushes and accessories", "10571EU"),
]
...
BC_TO_LEAF = {
    ...
    "Eyes": "pr-beauty-eyes",
    "Lips": "pr-beauty-lips",
    "Skincare": "pr-beauty-skincare",
    "Brushes and accessories": "pr-beauty-brushes",
    "Mother's Day": "pr-beauty-face",
}
LEAF_URL = {
    "pr-beauty-face": f"{BASE}/gb/en/perfumes-and-beauty/beauty/face/c/10568EU",
    "pr-beauty-eyes": f"{BASE}/gb/en/perfumes-and-beauty/beauty/eyes/c/10570EU",
    "pr-beauty-lips": f"{BASE}/gb/en/perfumes-and-beauty/beauty/lips/c/11102EU",
    "pr-beauty-skincare": f"{BASE}/gb/en/perfumes-and-beauty/beauty/skincare/c/10569EU",
    "pr-beauty-brushes": f"{BASE}/gb/en/perfumes-and-beauty/beauty/brushes-and-accessories/c/10571EU",
}

PARENT_COLS = ["prada", "prada-accessories", "pr-beauty"]
...
def infer_leaf_from_name(name: str) -> str | None:
    n = (name or "").lower()
    if any(x in n for x in ("lipstick", "lip ", "balm", "lip tint", "monochrome")):
        return "pr-beauty-lips"
    if any(x in n for x in ("mascara", "eyeliner", "eye ", "brow", "lash")):
        return "pr-beauty-eyes"
    if any(
        x in n
        for x in (
            "foundation",
        ...
        return "pr-beauty-face"
    if any(x in n for x in ("serum", "cream", "skincare", "moisturizer", "cleanser")):
        return "pr-beauty-skincare"
    if any(x in n for x in ("brush", "sponge", "pouch", "case")):
        return "pr-beauty-brushes"
    return None
...
def collections_for(hit: dict) -> list[str]:
    ...
    for cid in cats:
        ...
        if lid:
            cols.add(lid)
    if not any(c.startswith("pr-beauty-") for c in cols):
    ...
    if not any(c.startswith("pr-beauty-") for c in cols):
        ...
        cols.add(inferred or "pr-beauty-face")
    return sorted(cols)
...
def primary_leaf(cols: list[str], hit: dict) -> str:
    ...
    if mapped:
        return mapped
    name = ((hit.get("ProductName") or {}).get("en_GB") or "")
    return infer_leaf_from_name(name) or "pr-beauty-face"
...
def main() -> None:
    ...
    print("=== Prada Beauty (hub + leaves)", flush=True)
