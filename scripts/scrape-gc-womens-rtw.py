...
OUT_RAW = ROOT / "src/data/gc/gc-rtw-catalog-raw.json"
...
WARM_URL = f"{BASE}/uk/en_gb/ca/women/ready-to-wear-for-women-c-women-readytowear"

# Official Clothing leaves (Gucci UK RTW nav) → Briq collection ids
# categoryCode is the URL segment after -c-
COLLECTIONS: list[tuple[str, str, str]] = [
    ("gc-women-knitwear", "Knitwear", "women-readytowear-sweaters-cardigans"),
    ("gc-women-tops-shirts", "Tops & Shirts", "women-readytowear-tops-and-shirts"),
    (
        "gc-women-tshirts-sweatshirts",
        "T-Shirts & Sweatshirts",
        "women-t-shirts-sweatshirts",
    ),
    ("gc-women-dresses", "Dresses & Jumpsuits", "women-readytowear-dresses"),
    ("gc-women-pants-shorts", "Pants & Shorts", "women-readytowear-pants-and-shorts"),
    ("gc-women-denim", "Denim", "women-ready-to-wear-denim"),
    ("gc-women-skirts", "Skirts", "women-readytowear-skirts"),
    ("gc-women-swimwear", "Swimwear", "women-readytowear-swimwear"),
    ("gc-women-coats-jackets", "Coats & Jackets", "women-readytowear-coats-furs"),
    ("gc-women-outerwear", "Outerwear", "women-ready-to-wear-outerwear"),
    ("gc-women-leather", "Leather", "women-leather-jackets"),
    ("gc-women-activewear", "Activewear", "women-readytowear-activewear"),
    ("gc-women-cocktail-evening", "Cocktail & Evening", "women-cocktail-evening"),
]

ALL_RTW_CODE = "women-readytowear"
PARENT_COLL = "gc-women-rtw"
MAX_WORKERS = 6
IMG_WORKERS = 8
...
def upgrade_media_url(u: str) -> str:
    """Keep DarkGray studio crops — matches gucci.com RTW PLP/PDP look."""
    u = abs_url(u)
...
def download_image(s: cffi_requests.Session, url: str, dest: Path) -> bool:
    ...
    for attempt in range(3):
        try:
            ...
            # Do not whiten — RTW DarkGray studio mats must match gucci.com.
...
def extract_sizes(catalog: dict | None) -> list[dict]:
    """Return unique size rows from catalog variants (IT / letter sizes)."""
    out: list[dict] = []
    seen: set[str] = set()
    for v in (catalog or {}).get("variants") or []:
        size = str(v.get("sizeDescription") or "").strip()
        if not size or size.upper() in {"U", "OS", "ONE SIZE", "NS"}:
            continue
        key = size.upper()
        if key in seen:
            continue
...
def main() -> None:
    ...
    print(f"=== {PARENT_COLL} ({ALL_RTW_CODE})", flush=True)
    all_items = fetch_grid_category(s, ALL_RTW_CODE)
    plp_meta[PARENT_COLL] = {
        "label": "Clothing / View All",
        "categoryCode": ALL_RTW_CODE,
        "count": len({it["productCode"] for it in all_items}),
    }
    for it in all_items:
        code = it["productCode"]
        membership.setdefault(code, set()).update({PARENT_COLL, "gc-women", "gucci"})
        cards[code] = it
    ...
    for coll_id, label, cat_code in COLLECTIONS:
        ...
        for it in items:
            ...
            membership.setdefault(code, set()).update(
                {coll_id, PARENT_COLL, "gc-women", "gucci"}
            )
