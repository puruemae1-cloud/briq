#!/usr/bin/env python3
"""Scrape Prada GB men's shoes (~166) in stages.

Hub: https://www.prada.com/gb/en/mens/shoes/c/10149EU
PLP set = Algolia CategoriesEnriched ``10149EU|false|false``.

Official leaves: Loafers, Sneakers, Sandals, Lace-ups, Boots, America's Cup.

  python3 scripts/scrape-pr-mens-shoes.py --stage 1 --stages 2
  python3 scripts/scrape-pr-mens-shoes.py --refresh-sizes
"""
...
OUT_RAW = ROOT / "src/data/pr/pr-mens-shoes-catalog-raw.json"
PDP_CACHE = ROOT / "src/data/pr/pr-mens-shoes-pdp-cache.json"
SEED = ROOT / "src/data/pr/pr-mens-shoes-hub-seed.json"
IMG_ROOT = ROOT / "public/products/pr-pdp"
PROGRESS = ROOT / "src/data/pr/pr-mens-shoes-stage-progress.json"

BASE = "https://www.prada.com"
HUB_URL = f"{BASE}/gb/en/mens/shoes/c/10149EU"
HUB_CID = "10149EU"
...
# Official leaf PLPs under men's shoes (prada.com GB hub chips)
LEAVES: list[tuple[str, str, str]] = [
    ("pr-men-loafers", "Loafers", "10153EU"),
    ("pr-men-sneakers", "Sneakers", "10155EU"),
    ("pr-men-sandals", "Sandals", "10154EU"),
    ("pr-men-lace-ups", "Lace-ups", "10152EU"),
    ("pr-men-boots", "Boots", "10150EU"),
    ("pr-men-americas-cup", "America's Cup", "10297EU"),
]
LEAF_BY_CID = {cid: lid for lid, _label, cid in LEAVES}
BC_TO_LEAF = {
    "Loafers": "pr-men-loafers",
    "Sneakers": "pr-men-sneakers",
    "Sandals": "pr-men-sandals",
    "Lace-ups": "pr-men-lace-ups",
    "Boots": "pr-men-boots",
    "America's Cup": "pr-men-americas-cup",
    # Editorial PLPs that still sit under the shoes hub
    "Speedrock": "pr-men-sneakers",
    "Court": "pr-men-sneakers",
}
LEAF_SLUG = {
    "pr-men-loafers": "loafers",
    "pr-men-sneakers": "sneakers",
    "pr-men-sandals": "sandals",
    "pr-men-lace-ups": "lace-ups",
    "pr-men-boots": "boots",
    "pr-men-americas-cup": "americas-cup",
}

PARENT_COLS = ["prada", "prada-shoes", "pr-men-shoes"]
MAX_WORKERS = 4
IMG_WORKERS = 4
MAX_IMAGES = 10
...
def collections_for(hit: dict) -> list[str]:
    ...
    for cid in hit.get("Categories") or []:
        ...
        if lid:
            cols.add(lid)
    if not any(c.startswith("pr-men-") and c != "pr-men-shoes" for c in cols):
...
def main() -> None:
    ...
    for lid, label, cid in LEAVES:
        ...
        plp_meta[lid] = {
            ...
            "categoryCode": cid,
            "count": n,
            "url": f"{BASE}/gb/en/mens/shoes/{slug}/c/{cid}",
        }
